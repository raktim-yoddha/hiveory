...
export const CodeSourcePanel: React.FC<CodeSourcePanelProps> = ({ workspace, onClose }) => {
  ...
  const loadSource = useCallback(async () => {
    ...
    const [statusResult, repositoryResult, hostedResult] = await Promise.allSettled([
      agenticSuperAppClient.codeGitStatus({ workspace_id: workspace.id }),
      agenticSuperAppClient.codeGitRepository({ workspace_id: workspace.id }),
      agenticSuperAppClient.codeHostedTracking({ workspace_id: workspace.id }),
    ])
  ...
  const loadDiff = async (relativePath: string) => {
    ...
    setDiffLoading(true)
    try {
      setSelectedDiff(await agenticSuperAppClient.codeGitDiff({ workspace_id: workspace.id, relative_path: relativePath }))
