...
export const CodeCoordinationPanel: React.FC<CodeCoordinationPanelProps> = ({ workspace, onClose }) => {
  ...
  const loadRuns = useCallback(async () => {
    ...
    try {
      const [nextRuns, snapshot] = await Promise.all([
        agenticSuperAppClient.codeRuns(workspace.id),
        agenticSuperAppClient.codeSnapshot(),
      ])
      setRuns(nextRuns)
  ...
  const loadDetail = useCallback(async (runId: string) => {
    try {
      ...
      const [nextMailbox, nextGates] = await Promise.all([
        agenticSuperAppClient.codeMailbox({ run_id: runId, recipient_address: coordinatorAddress, include_acknowledged: true, limit: 80 }),
        agenticSuperAppClient.codeGates({ run_id: runId, include_resolved: true }),
      ])
  ...
  useEffect(() => {
    ...
    return agenticSuperAppClient.subscribeCodeOrchestration(selectedRunId, () => {
      void loadDetail(selectedRunId)
      void loadRuns()
    })
  ...
  const createRun = async () => {
    ...
    setError(null)
    try {
      const nextDetail = await agenticSuperAppClient.createCodeRun({
  ...
  const draftDag = async () => {
    ...
    setError(null)
    try {
      const proposal = await agenticSuperAppClient.proposeCodeDag({ workspace_id: workspace.id, objective: detail.summary.objective, model: detail.summary.model })
  ...
  const acceptDag = async () => {
    ...
    await runAction('accept-dag', () => agenticSuperAppClient.acceptCodeDag({ run_id: detail.summary.id, proposal: detail.proposal! }))
  }

  const addTask = async () => {
    if (!detail || !taskTitle.trim() || !taskSpecification.trim()) return
    await runAction('add-task', () => agenticSuperAppClient.createCodeTask({ run_id: detail.summary.id, client_id: null, title: taskTitle.trim(), specification: taskSpecification.trim(), depends_on: [] }))
    setTaskTitle('')
  ...
  const answerQuestion = async (question: CodeQuestion) => {
    ...
    if (!dispatch) return
    await runAction('answer', () => agenticSuperAppClient.answerCodeQuestion({ run_id: detail.summary.id, task_id: question.task_id, dispatch_id: question.dispatch_id, lease_generation: dispatch.lease_generation, answer: questionAnswer.trim() }))
    setQuestionAnswer('')
  }
  ...
  const sendMailboxMessage = async () => {
    ...
    setError(null)
    try {
      await agenticSuperAppClient.sendCodeMailbox({
      ...
      setMessagePayload('')
      setMailbox(await agenticSuperAppClient.codeMailbox({ run_id: detail.summary.id, recipient_address: `coordinator:${detail.summary.coordinator_id}`, include_acknowledged: true, limit: 80 }))
    } catch (reason: unknown) {
      setError(reason instanceof Error ? reason.message : String(reason))
    } finally {
      setBusy(null)
  ...
  const acknowledgeMailbox = async (delivery: CodeMailboxDelivery) => {
    ...
    await runAction(`ack-${delivery.id}`, async () => {
      await agenticSuperAppClient.acknowledgeCodeMailbox({ run_id: detail.summary.id, delivery_id: delivery.id, recipient_address: delivery.recipient_address })
      await loadDetail(detail.summary.id)
      return agenticSuperAppClient.codeRun(detail.summary.id)
    })
  }
  ...
  const openGate = async () => {
    ...
    setError(null)
    try {
      await agenticSuperAppClient.createCodeGate({ run_id: detail.summary.id, task_id: null, dispatch_id: null, title: gateTitle.trim(), reason: gateReason.trim(), allowed_actor: 'user', expires_at_unix_ms: null })
      setGateTitle('')
      setGateReason('')
      setGates(await agenticSuperAppClient.codeGates({ run_id: detail.summary.id, include_resolved: true }))
    } catch (reason: unknown) {
      setError(reason instanceof Error ? reason.message : String(reason))
    } finally {
      setBusy(null)
  ...
  const resolveGate = async (gate: CodeDecisionGate, state: Exclude<CodeGateState, 'open'>) => {
    ...
    setError(null)
    try {
      await agenticSuperAppClient.resolveCodeGate({ run_id: detail.summary.id, gate_id: gate.id, actor: 'user', state, resolution: state === 'approved' ? 'Approved from the coordination surface.' : state === 'rejected' ? 'Rejected from the coordination surface.' : 'Timed out by the coordinator.' })
      setGates(await agenticSuperAppClient.codeGates({ run_id: detail.summary.id, include_resolved: true }))
    } catch (reason: unknown) {
      setError(reason instanceof Error ? reason.message : String(reason))
    } finally {
    ...
    <aside className="code-coordination-panel" aria-label="Coordination">
      <header className="code-coordination-panel-header">
        <div className="code-coordination-panel-title">
          ...
          <div><strong>Coordination</strong><span>{workspace.display_name} · {agenticSuperAppClient.isTauri ? 'local host' : 'browser preview'}</span></div>
        </div>
      ...
      <div className="code-coordination-body">
        ...
        {detail && <section className="code-coordination-detail" aria-labelledby="code-coordination-detail-title">
          ...
          <div className="code-coordination-actions">
            {['draft', 'ready', 'paused', 'blocked'].includes(detail.summary.state) && <button type="button" onClick={() => void runAction('start', () => agenticSuperAppClient.startCodeRun({ run_id: detail.summary.id }))} disabled={busy !== null}><Play size={12} aria-hidden="true" />Start</button>}
            {detail.summary.state === 'running' && <button type="button" className="is-secondary" onClick={() => void runAction('pause', () => agenticSuperAppClient.pauseCodeRun({ run_id: detail.summary.id }))} disabled={busy !== null}><Pause size={12} aria-hidden="true" />Pause</button>}
            {!['completed', 'cancelled'].includes(detail.summary.state) && <button type="button" className="is-danger" onClick={() => void runAction('cancel', () => agenticSuperAppClient.cancelCodeRun({ run_id: detail.summary.id }))} disabled={busy !== null}><Square size={11} aria-hidden="true" />Cancel</button>}
          ...
          <div className="code-coordination-worker-list">{detail.dispatches.length === 0 ? <p>No worker dispatches yet. Start the run after adding tasks.</p> : detail.dispatches.slice(0, 8).map((dispatch) => <WorkerRow key={dispatch.id} dispatch={dispatch} task={detail.tasks.find((task) => task.id === dispatch.task_id)} busy={busy !== null} onCancel={() => void runAction(`cancel-${dispatch.id}`, () => agenticSuperAppClient.cancelCodeDispatch({ run_id: detail.summary.id, task_id: dispatch.task_id, dispatch_id: dispatch.id, lease_generation: dispatch.lease_generation }))} onResume={() => void runAction(`resume-${dispatch.id}`, () => agenticSuperAppClient.resumeCodeDispatch({ run_id: detail.summary.id, task_id: dispatch.task_id, dispatch_id: dispatch.id, lease_generation: dispatch.lease_generation }))} />)}</div>
