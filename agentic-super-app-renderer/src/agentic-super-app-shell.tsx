import { Activity, Bell, Bot, CheckCircle2, Code2, Command, Download, FolderArchive, KeyRound, Keyboard, MessageSquare, PanelLeft, Play, RotateCcw, Settings2, ShieldCheck, Square, Wifi, X } from 'lucide-react'
...
export function AgenticSuperAppShell() {
  ...
  const selectMode = (mode: ApplicationMode) => { setScreen('workspace'); setNotificationsOpen(false); if (mode === 'code') setCodeScreen('workbench'); if (mode === 'agent') setAgentScreen('workspace'); setActiveMode(mode); void agenticSuperAppClient.setActiveMode(mode).then((item) => setActiveMode(item.active_mode)).catch(() => undefined) }
  ...
  return <>
    ...
    <section id="agentic-super-app-main-content" tabIndex={-1} className={`agentic-super-app-workspace ${screen === 'workspace' && activeMode === 'chat' ? 'is-chat-mode' : ''} ${screen === 'workspace' && activeMode === 'code' ? 'is-code-mode' : ''}`}>
      ...
      {screen === 'diagnostics' ? <AgenticSuperAppDiagnostics snapshot={snapshot} events={events} refresh={refresh} /> : screen === 'settings' ? <AgenticSuperAppSettings preferences={preferences} setPreferences={setPreferences} onOpenDiagnostics={() => { setScreen('diagnostics'); void refresh() }} /> : activeMode === 'chat' ? <AgenticSuperAppChat /> : activeMode === 'code' ? <Suspense fallback={<section className="agentic-super-app-content" role="status">Loading Code workspace…</section>}>{codeScreen === 'runs' ? <AgenticSuperAppCodeRuns /> : <AgenticSuperAppCode />}</Suspense> : agentScreen === 'routines' ? <AgenticSuperAppRoutines /> : agentScreen === 'plugins' ? <AgenticSuperAppPlugins /> : <Suspense fallback={<section className="agentic-super-app-content" role="status">Loading Agent workspace…</section>}><AgenticSuperAppAgent initialPanel={agentPanel} /></Suspense>}
    </section>
