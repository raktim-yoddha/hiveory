...
export function AgenticSuperAppCodeRuns() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = agenticSuperAppClient.subscribeCodeOrchestration(selectedRunId, (event) => {
  ...
  const showDiff = async () => {
    ...
    setBusy('diff')
    try {
      const diff = await agenticSuperAppClient.codeCheckpointDiff({ run_id: detail.summary.id, checkpoint_id: selectedTask.latest_checkpoint_id, compare_to_checkpoint_id: null })
  ...
  const openTerminal = async (dispatch: CodeDispatch) => {
    ...
    try {
      const summary = await agenticSuperAppClient.openCodeDispatchTerminal({ run_id: detail.summary.id, dispatch_id: dispatch.id, cols: 100, rows: 28 }, (event) => handleTerminalEvent(event))
  ...
  const cancelDispatch = async (dispatch: CodeDispatch) => {
    if (!detail) return
    await runAction(`cancel-dispatch-${dispatch.id}`, () => agenticSuperAppClient.cancelCodeDispatch({ run_id: detail.summary.id, task_id: dispatch.task_id, dispatch_id: dispatch.id, lease_generation: dispatch.lease_generation }))
  }

  const resumeDispatch = async (dispatch: CodeDispatch) => {
    if (!detail) return
    await runAction(`resume-dispatch-${dispatch.id}`, () => agenticSuperAppClient.resumeCodeDispatch({ run_id: detail.summary.id, task_id: dispatch.task_id, dispatch_id: dispatch.id, lease_generation: dispatch.lease_generation }))
  }
      ...
      <main className="agentic-super-app-runs-main">
        {!detail ? <div className="agentic-super-app-runs-empty"><div className="agentic-super-app-empty-mark"><Workflow size={28} /></div><h2>Coordinate work in reviewable lanes</h2><p>Each run owns a durable DAG, isolated worker worktrees, checkpoints, and explicit review decisions.</p></div> : <>
          ...
          <div className="agentic-super-app-runs-content-grid">
            ...
            <aside className="agentic-super-app-run-inspector" aria-label="Selected task inspector"><div className="agentic-super-app-run-section-heading"><div><GitBranch size={15} /><h3>Inspector</h3></div></div>{selectedTask ? <TaskInspector task={selectedTask} detail={detail} onReview={review} reviewFeedback={reviewFeedback} setReviewFeedback={setReviewFeedback} onDiff={() => void showDiff()} diff={checkpointDiff} diffBusy={busy === 'diff'} /> : <p className="agentic-super-app-code-muted">Select a task to inspect its specification, worker lease, checkpoint, or review.</p>}</aside>
          </div>
          <section className="agentic-super-app-worker-lanes" aria-labelledby="agentic-super-app-worker-lanes-title"><div className="agentic-super-app-run-section-heading"><div><Bot size={15} /><h3 id="agentic-super-app-worker-lanes-title">Worker lanes</h3><span>{runningDispatches.length} active</span></div><span className="agentic-super-app-code-muted">Concurrency {detail.summary.active_dispatches}/{Math.min(detail.summary.concurrency_limit, detail.summary.host_concurrency_cap)}</span></div>{detail.dispatches.length ? <div className="agentic-super-app-worker-lane-grid">{detail.dispatches.slice(0, 8).map((dispatch) => <WorkerLane key={dispatch.id} dispatch={dispatch} task={detail.tasks.find((task) => task.id === dispatch.task_id)} busy={busy !== null} onCancel={() => void cancelDispatch(dispatch)} onResume={() => void resumeDispatch(dispatch)} onOpenTerminal={() => void openTerminal(dispatch)} />)}</div> : <p className="agentic-super-app-code-muted">Workers appear here after the run starts. Every worker receives an isolated managed worktree.</p>}</section>
          {terminalId && <section className="agentic-super-app-run-terminal" aria-labelledby="agentic-super-app-run-terminal-title"><div className="agentic-super-app-run-section-heading"><div><Terminal size={15} /><h3 id="agentic-super-app-run-terminal-title">Attached dispatch terminal</h3></div><span className="agentic-super-app-code-muted"><code>{terminalId}</code></span></div><pre aria-live="polite">{terminalOutput || 'Terminal connected. Worker output will appear here.'}</pre></section>}
          <section className="agentic-super-app-run-events" aria-labelledby="agentic-super-app-run-events-title"><div className="agentic-super-app-run-section-heading"><div><Activity size={15} /><h3 id="agentic-super-app-run-events-title">Event timeline</h3><span>{detail.events.length} retained</span></div><span className="agentic-super-app-code-muted">Cursor {detail.event_cursor}</span></div><div className="agentic-super-app-run-event-list" aria-live="polite">{detail.events.slice(-12).reverse().map((event) => <div key={event.event_id}><span className={`agentic-super-app-event-origin ${event.origin}`}>{event.origin}</span><code>#{event.sequence}</code><span>{event.payload}</span><small>{event.kind.replaceAll('_', ' ')}</small></div>)}</div></section>
          <div className="agentic-super-app-sr-only" aria-live="polite">{liveAnnouncement}</div>
...
function WorkerLane({ dispatch, task, busy, onCancel, onResume, onOpenTerminal }: { dispatch: CodeDispatch; task?: CodeTask; busy: boolean; onCancel: () => void; onResume: () => void; onOpenTerminal: () => void }) {
  ...
  return <article className="agentic-super-app-worker-lane"><div className="agentic-super-app-worker-lane-top"><span className="agentic-super-app-worker-pulse" aria-hidden="true" /><strong>{task?.title ?? 'Unknown task'}</strong><span className={`agentic-super-app-run-badge ${dispatch.state}`}>{dispatch.state.replaceAll('_', ' ')}</span></div><div className="agentic-super-app-worker-lane-meta"><span>Attempt {dispatch.attempt}</span><span>{dispatch.session_id ? 'resumable session' : 'new session'}</span><span>{dispatch.pid ? `PID ${dispatch.pid}` : 'awaiting process'}</span><span>Lease {dispatch.lease_generation}</span>{dispatch.last_heartbeat_at_unix_ms && <span>Heartbeat {new Date(dispatch.last_heartbeat_at_unix_ms).toLocaleTimeString()}</span>}</div>{dispatch.result_summary && <p>{dispatch.result_summary}</p>}{dispatch.error && <p className="agentic-super-app-worker-error">{dispatch.error}</p>}<div className="agentic-super-app-worker-lane-actions">{active && <button className="is-secondary" onClick={onCancel} disabled={busy}><Square size={12} />Cancel dispatch</button>}{dispatch.state === 'interrupted' && <button onClick={onResume} disabled={busy}><RotateCcw size={12} />Resume</button>}{dispatch.worktree_id && ['preparing', 'running', 'awaiting_input', 'checkpointing', 'interrupted'].includes(dispatch.state) && <button className="is-secondary" onClick={onOpenTerminal} disabled={busy}><Terminal size={12} />Open terminal</button>}</div></article>
}
...
function TaskInspector({ task, detail, onReview, reviewFeedback, setReviewFeedback, onDiff, diff, diffBusy }: { task: CodeTask; detail: CodeRunDetail; onReview: (decision: CodeReviewDecision) => Promise<void>; reviewFeedback: string; setReviewFeedback: (value: string) => void; onDiff: () => void; diff: string | null; diffBusy: boolean }) {
  ...
  return <div className="agentic-super-app-task-inspector"><div className="agentic-super-app-task-inspector-title"><span className={`agentic-super-app-task-status ${task.state}`} /><div><h4>{task.title}</h4><p>{task.client_id} · attempt {task.attempt}</p></div></div><dl><div><dt>State</dt><dd>{task.state.replaceAll('_', ' ')}</dd></div><div><dt>Worker</dt><dd>{dispatch?.state.replaceAll('_', ' ') ?? 'not dispatched'}</dd></div><div><dt>Checkpoint</dt><dd>{checkpoint ? checkpoint.kind : 'none'}</dd></div><div><dt>Lease</dt><dd>{dispatch ? `generation ${dispatch.lease_generation}` : 'none'}</dd></div><div><dt>Terminal</dt><dd>{dispatch?.terminal_id ? 'attached' : 'none'}</dd></div></dl><div className="agentic-super-app-task-spec"><span>Specification</span><p>{task.specification}</p></div>{task.error && <div className="agentic-super-app-task-error"><CircleAlert size={14} />{task.error}</div>}{checkpoint && <div className="agentic-super-app-checkpoint"><div><span>Checkpoint {checkpoint.kind}</span><code>{checkpoint.commit_oid?.slice(0, 10) ?? 'pending'}</code></div><button className="is-secondary" onClick={onDiff} disabled={diffBusy}>{diffBusy ? 'Loading diff…' : 'View checkpoint diff'}</button>{diff && <pre>{diff}</pre>}</div>}{canReview && <div className="agentic-super-app-review-form"><label htmlFor="agentic-super-app-review-feedback">Review feedback</label><textarea id="agentic-super-app-review-feedback" rows={3} value={reviewFeedback} onChange={(event) => setReviewFeedback(event.target.value)} placeholder="Optional acceptance notes or requested changes" /><div><button onClick={() => void onReview('accept')}><Check size={14} />Accept</button><button className="is-secondary" onClick={() => void onReview('request_changes')}><RotateCcw size={14} />Request changes</button><button className="is-danger" onClick={() => void onReview('reject')}><X size={14} />Reject</button></div></div>}</div>
}
