...
impl HiveoryPersistence {
    pub async fn send_orchestration_mailbox(
            ...
            .filter(|value| !value.is_empty())
        {
            if let Some(row) = sqlx::query(
                "SELECT id, run_id, sender_address, recipient_address, kind, payload, thread_id, delivery_sequence, acknowledged, created_at_unix_ms, acknowledged_at_unix_ms FROM hiveory_code_mailbox_deliveries WHERE run_id=? AND client_request_id=?",
            )
            .bind(&request.run_id)
        ...
        let updated = sqlx::query(
            "UPDATE hiveory_code_participants SET next_delivery_sequence=next_delivery_sequence+1, updated_at_unix_ms=? WHERE run_id=? AND address=?",
        )
        .bind(now)
        .bind(&request.run_id)
        ...
        let sequence: i64 = sqlx::query(
            "SELECT next_delivery_sequence - 1 FROM hiveory_code_participants WHERE run_id=? AND address=?",
        )
        .bind(&request.run_id)
        .bind(&request.recipient_address)
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_mailbox_deliveries (id, run_id, sender_address, recipient_address, kind, payload, thread_id, delivery_sequence, acknowledged, created_at_unix_ms, acknowledged_at_unix_ms, client_request_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, NULL, ?)",
        )
        .bind(&delivery.id)
    ...
    pub async fn orchestration_inbox(
        ...
        let rows = if query.include_acknowledged {
            sqlx::query(
                "SELECT id, run_id, sender_address, recipient_address, kind, payload, thread_id, delivery_sequence, acknowledged, created_at_unix_ms, acknowledged_at_unix_ms FROM hiveory_code_mailbox_deliveries WHERE run_id=? AND recipient_address=? ORDER BY delivery_sequence LIMIT ?",
            )
            .bind(&query.run_id)
            ...
            sqlx::query(
                "SELECT id, run_id, sender_address, recipient_address, kind, payload, thread_id, delivery_sequence, acknowledged, created_at_unix_ms, acknowledged_at_unix_ms FROM hiveory_code_mailbox_deliveries WHERE run_id=? AND recipient_address=? AND acknowledged=0 ORDER BY delivery_sequence LIMIT ?",
            )
            .bind(&query.run_id)
            .bind(&query.recipient_address)
            .bind(limit)
    ...
    pub async fn acknowledge_orchestration_mailbox(
        ...
        Ok(sqlx::query(
            "UPDATE hiveory_code_mailbox_deliveries SET acknowledged=1, acknowledged_at_unix_ms=? WHERE id=? AND run_id=? AND recipient_address=? AND acknowledged=0",
        )
        .bind(now_ms())
        .bind(delivery_id)
        .bind(run_id)
    ...
    pub async fn orchestration_participants(
        ...
        Ok(sqlx::query(
            "SELECT id, run_id, address, kind, display_name, active, created_at_unix_ms, updated_at_unix_ms FROM hiveory_code_participants WHERE run_id=? ORDER BY created_at_unix_ms, address",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
...
async fn ensure_participant(
    ...
    sqlx::query(
        "INSERT INTO hiveory_code_participants (id, run_id, address, kind, display_name, active, next_delivery_sequence, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, 1, 1, ?, ?) ON CONFLICT(run_id, address) DO UPDATE SET active=1, updated_at_unix_ms=excluded.updated_at_unix_ms",
    )
...
mod tests {
    ...
    async fn fixture() -> (HiveoryPersistence, String, String, std::path::PathBuf) {
        let path = std::env::temp_dir().join(format!("hiveory-mailbox-{}.sqlite3", Uuid::now_v7()));
