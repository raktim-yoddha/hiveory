...
#[derive(Debug, Error)]
pub enum HiveoryCodeOrchestrationError {
    #[error("database operation failed: {0}")]
    Database(#[from] sqlx::Error),
    #[error("workspace operation failed: {0}")]
    Workspace(#[from] HiveoryWorkspaceError),
    #[error("Git operation failed: {0}")]
    Git(#[from] HiveoryGitError),
...
pub type HiveoryCodeOrchestrationResult<T> = Result<T, HiveoryCodeOrchestrationError>;
...
impl HiveoryCodeOrchestration {
    ...
    pub async fn recover(&self) -> HiveoryCodeOrchestrationResult<usize> {
    ...
    ) -> HiveoryCodeOrchestrationResult<Vec<CodeRunSummary>> {
    ...
    pub async fn detail(
        ...
        self.persistence
            ...
            .await?
            .ok_or(HiveoryCodeOrchestrationError::NotFound)
    }

    pub async fn send_mailbox_message(
        &self,
        request: &CodeMailboxSendRequest,
    ) -> HiveoryCodeOrchestrationResult<CodeMailboxDelivery> {
    ...
    ) -> HiveoryCodeOrchestrationResult<Vec<CodeMailboxDelivery>> {
    ...
    ) -> HiveoryCodeOrchestrationResult<bool> {
        validate_participant_address(&request.recipient_address)?;
        self.detail(&request.run_id).await?;
        Ok(self
            .persistence
    ...
    ) -> HiveoryCodeOrchestrationResult<Vec<CodeDecisionGate>> {
        self.detail(&query.run_id).await?;
        Ok(self
            .persistence
    ...
    pub async fn create_gate(
    ...
    ) -> HiveoryCodeOrchestrationResult<CodeDecisionGate> {
        ...
        {
            return Err(HiveoryCodeOrchestrationError::NotFound);
        }
    ...
    pub async fn resolve_gate(
        ...
        let gate = self
            ...
            .await?
            .ok_or_else(|| {
                HiveoryCodeOrchestrationError::InvalidState(
    ...
    pub async fn create_run(
        ...
        if !self.worker_adapter.supports(adapter_id) {
            return Err(HiveoryCodeOrchestrationError::InvalidState(format!(
                "worker adapter {adapter_id} is not installed"
            )));
        }
    ...
    pub async fn create_task(
        ...
        if detail.tasks.len() >= hiveory_code_domain::CODE_MAX_ORCHESTRATION_TASKS {
    ...
    ) -> HiveoryCodeOrchestrationResult<CodeDagProposal> {
        validate_orchestration_text(&request.objective)?;
        self.workspaces
            ...
            let dependencies = proposal_task
                ...
                .map(|dependency| {
                    let dependency_id = ids_by_client.get(dependency).ok_or_else(|| {
                        HiveoryCodeOrchestrationError::InvalidState(format!(
                ...
                .collect::<HiveoryCodeOrchestrationResult<Vec<_>>>()?;
            let now = now_ms();
            self.persistence
    ...
    pub async fn resume_dispatch(
        ...
        let dispatch = detail
            ...
            .cloned()
            .ok_or(HiveoryCodeOrchestrationError::InvalidState(
    ...
    ) -> HiveoryCodeOrchestrationResult<CodeDispatchTerminalContext> {
        let detail = self.detail(&request.run_id).await?;
        self.require_worker_capabilities(&detail.summary.workspace_id)?;
        let dispatch = detail
            .dispatches
    ...
    ) -> HiveoryCodeOrchestrationResult<CodeCleanupPreview> {
        let detail = self.detail(&request.run_id).await?;
        self.workspaces.require(
    ...
    pub async fn checkpoint_diff(
    ...
    ) -> HiveoryCodeOrchestrationResult<hiveory_protocol::CodeGitDiff> {
        let detail = self.detail(&request.run_id).await?;
        self.workspaces.require(
            &detail.summary.workspace_id,
            CodeWorkspaceCapability::ReadGit,
        ...
        let to = checkpoint.commit_oid.as_deref().ok_or_else(|| {
            HiveoryCodeOrchestrationError::InvalidState("checkpoint has no commit".to_owned())
        })?;
        let root = self.workspaces.root_path(&detail.summary.workspace_id)?;
    ...
    pub async fn cleanup_confirm(
        ...
        if request.confirmation.trim() != expected {
            return Err(HiveoryCodeOrchestrationError::InvalidCleanupConfirmation);
        }
        if request.force {
    ...
    pub fn verify_worker_event(
        secret: &[u8],
        event_json: &str,
    ) -> HiveoryCodeOrchestrationResult<VerifiedWorkerEvent> {
        ...
        if event.dispatch_id.is_empty()
            ...
            || event.mac.len() > 128
        {
            return Err(HiveoryCodeOrchestrationError::InvalidWorkerEvent);
        }
        ...
        let mut mac = HmacSha256::new_from_slice(secret)
            .map_err(|_| HiveoryCodeOrchestrationError::InvalidWorkerEvent)?;
    ...
    async fn run_proposal_worker(
        ...
        let output = output
            .map_err(|error| HiveoryCodeOrchestrationError::WorkerUnavailable(error.to_string()))?;
        if !output.status.success() {
            return Err(HiveoryCodeOrchestrationError::WorkerFailed(
                String::from_utf8_lossy(&output.stderr).trim().to_owned(),
            ));
        }
        parse_proposal_output(&String::from_utf8_lossy(&output.stdout)).ok_or_else(|| {
            HiveoryCodeOrchestrationError::WorkerFailed(
    ...
    async fn ensure_source_checkpoint(
        &self,
        detail: &hiveory_protocol::CodeRunDetail,
    ) -> HiveoryCodeOrchestrationResult<CodeCheckpoint> {
        ...
        let checkpoint = CodeCheckpoint {
            ...
            ref_name: format!("hiveory/source/{}", short_id(&detail.summary.id)),
    ...
    async fn schedule_once(&self, run_id: &str) -> HiveoryCodeOrchestrationResult<bool> {
    ...
    async fn prepare_and_launch(
        ...
        let base = match self.resolve_base_checkpoint(&detail, &task).await {
            ...
            Err(error) => {
                ...
                if matches!(
                    &error,
                    HiveoryCodeOrchestrationError::Git(HiveoryGitError::MergeConflict(_))
                ) {
    ...
    async fn run_codex_worker(
        ...
        launch: &WorkerLaunch,
        cancellation: CancellationToken,
    ) -> HiveoryCodeOrchestrationResult<WorkerResult> {
        ...
        command
            .arg("-")
            .env("HIVEORY_DISPATCH_SECRET", hex_encode(&launch.secret))
            .env("HIVEORY_DISPATCH_ID", &launch.dispatch.id)
            .env(
                "HIVEORY_DISPATCH_LEASE",
                launch.dispatch.lease_generation.to_string(),
            )
            .env("HIVEORY_DISPATCH_SEQUENCE", "0")
            .env("HIVEORY_DISPATCH_BRIDGE", dispatch_bridge_program())
                ...
                line = line_receiver.recv() => {
                    match line {
                        Some(WorkerOutputLine::Text(line)) => {
                            ...
                            if let Some(event_json) = line.strip_prefix("HIVEORY_EVENT ") {
        ...
        let stderr = stderr_reader
            .await
            .map_err(|error| HiveoryCodeOrchestrationError::WorkerFailed(error.to_string()))??;
        if let Some(error) = stream_error {
            return Err(HiveoryCodeOrchestrationError::WorkerFailed(error));
        }
    ...
    async fn accept_worker_event(
        ...
        let kind = parse_message_kind(&event.kind)
            .ok_or(HiveoryCodeOrchestrationError::InvalidWorkerEvent)?;
    ...
    async fn finish_worker(
        ...
        if let Some(question) = result.question {
            ...
            let question_row = hiveory_protocol::CodeQuestion {
    ...
    async fn reconcile_run(&self, run_id: &str) -> HiveoryCodeOrchestrationResult<()> {
        let detail = self.detail(run_id).await?;
        if matches!(
            detail.summary.state,
    ...
    async fn emit_status(
        ...
        payload: &str,
        accepted: bool,
    ) -> HiveoryCodeOrchestrationResult<CodeOrchestrationEventEnvelope> {
...
fn dispatch_bridge_program() -> String {
    let executable_name = if cfg!(windows) {
        "hiveory-dispatch-bridge.exe"
    } else {
        "hiveory-dispatch-bridge"
    };
...
fn ensure_editable_run(state: &CodeRunState) -> HiveoryCodeOrchestrationResult<()> {
    if matches!(
        state,
        CodeRunState::Draft
            | CodeRunState::Ready
        ...
        Err(HiveoryCodeOrchestrationError::InvalidState(
...
fn resolve_dependency_ids(
    ...
    requested: &[String],
    task_id: &str,
) -> HiveoryCodeOrchestrationResult<Vec<CodeTaskDependency>> {
    requested
        .iter()
        .map(|dependency| {
...
fn validate_proposal(proposal: &CodeDagProposal) -> HiveoryCodeOrchestrationResult<()> {
...
fn parse_worker_output(stdout: &str, stderr: &str, process_success: bool) -> WorkerResult {
    ...
    for line in stdout.lines() {
        let line = line.trim();
        if let Some(question_text) = line.strip_prefix("HIVEORY_QUESTION:") {
...
fn worker_prompt(task: &CodeTask, answer: Option<&str>) -> String {
    let answer = answer
        ...
        .unwrap_or_default();
    format!(
        "You are a bounded implementation worker inside an Hiveory run. Work only in the provided Git worktree. Do not modify the parent workspace, change remotes, or create pull requests. Inspect the repository, implement the task, and validate it with focused tests when practical. If a decision is required, stop and print exactly `HIVEORY_QUESTION: <question>` rather than guessing. For progress or escalation messages, you may invoke the executable in `$HIVEORY_DISPATCH_BRIDGE` with `<kind> <payload> <sequence>`; never print or expose the dispatch secret, and continue if the bridge is unavailable.\n\nTask: {}\n\nSpecification:\n{}\n{}\nAt the end, report a concise summary of changes and validation.",
        task.title, task.specification, answer
    )
}

fn hex_decode(value: &str) -> HiveoryCodeOrchestrationResult<Vec<u8>> {
...
fn validate_participant_address(value: &str) -> HiveoryCodeOrchestrationResult<()> {
...
mod tests {
    ...
    fn bridge_event_signature_round_trips() {
        ...
        let event =
            HiveoryCodeOrchestration::verify_worker_event(secret, &json.to_string()).unwrap();
        assert_eq!(event.dispatch_id, dispatch_id);
        assert!(
            HiveoryCodeOrchestration::verify_worker_event(b"wrong", &json.to_string()).is_err()
        );
    }
    ...
    async fn denies_worker_planning_for_an_untrusted_workspace() {
        ...
        let workspace = workspaces
            .open_workspace(
                &workspace_root,
                None,
                hiveory_protocol::CodeWorkspaceTrust::Untrusted,
            )
            .unwrap();
        let service = HiveoryCodeOrchestration::new(
            persistence,
        ...
        assert!(matches!(
            result,
            Err(HiveoryCodeOrchestrationError::Workspace(
                HiveoryWorkspaceError::CapabilityDenied(CodeWorkspaceCapability::ExecuteProcesses)
            ))
        ));
    }
