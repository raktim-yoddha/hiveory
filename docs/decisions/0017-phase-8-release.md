...
- The Tauri host owns update checks and installation. A build is considered update-enabled only when it receives an HTTPS endpoint and a signing public key through `AGENTIC_SUPER_APP_UPDATER_ENDPOINT` and `AGENTIC_SUPER_APP_UPDATER_PUBKEY`. The updater plugin is initialized for every build, but an unconfigured local build performs no network request.
...
- `cargo test -p agentic-super-app-app-host` covers archive path safety and manifest validation.
