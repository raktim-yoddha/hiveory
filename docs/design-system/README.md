...
Vetted third-party brand SVGs may retain their official colors only through `shared/ui/HiveoryBrandIcon.tsx`; all surrounding interface chrome remains graphite and must use the normal tokens.
...
The sibling `hiveory-private` repository is not a second design system. Its
Agent and Auto Plugin surfaces must use these public tokens and shared
primitives wherever they fit, while retaining `compact` density for utility
areas. Its premium theme stylesheet is the only approved private exception to
hue-neutral interface chrome: those chromatic presets are Dev-only,
local-profile-only prototypes and must never enter public CSS or Production.
