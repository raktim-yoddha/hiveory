...
impl AgenticSuperAppPersistence {
    pub async fn create_orchestration_gate(
        ...
        sqlx::query(
            "INSERT INTO agentic_super_app_code_decision_gates (id, run_id, task_id, dispatch_id, title, reason, state, allowed_actor, resolved_by, resolution, expires_at_unix_ms, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, 'open', ?, NULL, NULL, ?, ?, ?)",
        )
        .bind(&gate.id)
        .bind(&gate.run_id)
        .bind(&gate.task_id)
    ...
    pub async fn orchestration_gates(
        ...
        sqlx::query(
            "UPDATE agentic_super_app_code_decision_gates SET state='timed_out', resolved_by='system', resolution='Gate expired before resolution.', updated_at_unix_ms=? WHERE run_id=? AND state='open' AND expires_at_unix_ms IS NOT NULL AND expires_at_unix_ms<=?",
        )
        .bind(now_ms())
        .bind(run_id)
        .bind(now_ms())
        ...
        let rows = if include_resolved {
            sqlx::query(
                "SELECT id, run_id, task_id, dispatch_id, title, reason, state, allowed_actor, resolved_by, resolution, expires_at_unix_ms, created_at_unix_ms, updated_at_unix_ms FROM agentic_super_app_code_decision_gates WHERE run_id=? ORDER BY CASE WHEN state='open' THEN 0 ELSE 1 END, updated_at_unix_ms DESC",
            )
            .bind(run_id)
            .fetch_all(self.pool())
            .await?
            ...
            sqlx::query(
                "SELECT id, run_id, task_id, dispatch_id, title, reason, state, allowed_actor, resolved_by, resolution, expires_at_unix_ms, created_at_unix_ms, updated_at_unix_ms FROM agentic_super_app_code_decision_gates WHERE run_id=? AND state='open' ORDER BY updated_at_unix_ms DESC",
            )
            .bind(run_id)
            .fetch_all(self.pool())
            .await?
    ...
    pub async fn resolve_orchestration_gate(
        ...
        let updated = sqlx::query(
            "UPDATE agentic_super_app_code_decision_gates SET state=?, resolved_by=?, resolution=?, updated_at_unix_ms=? WHERE id=? AND run_id=? AND state='open' AND (allowed_actor='*' OR allowed_actor=?)",
        )
        ...
        sqlx::query(
            "SELECT id, run_id, task_id, dispatch_id, title, reason, state, allowed_actor, resolved_by, resolution, expires_at_unix_ms, created_at_unix_ms, updated_at_unix_ms FROM agentic_super_app_code_decision_gates WHERE id=?",
        )
        .bind(&request.gate_id)
