use super::AgenticSuperAppPersistence;
...
#[derive(Debug, Error)]
pub enum AgenticSuperAppChatStoreError {
...
#[derive(Debug, Clone)]
pub struct AgenticSuperAppChatTurnStart {
...
#[derive(Debug, Clone)]
pub struct AgenticSuperAppChatStoredAttachment {
...
#[derive(Clone)]
pub struct AgenticSuperAppChatStore {
    persistence: AgenticSuperAppPersistence,
}

impl AgenticSuperAppChatStore {
    ...
    pub async fn create(
    ...
    ) -> Result<ChatConversationDetail, AgenticSuperAppChatStoreError> {
        if let Some(command_request_id) = command_request_id {
            if let Some(row) = sqlx::query(
                "SELECT id FROM agentic_super_app_chat_conversations WHERE command_request_id=?",
            )
        ...
        sqlx::query("INSERT INTO agentic_super_app_chat_conversations (id, title, active_branch_id, command_request_id, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?)")
            .bind(&conversation_id).bind(&title).bind(&branch_id).bind(command_request_id).bind(now).bind(now)
            .execute(&mut *tx).await?;
        sqlx::query("INSERT INTO agentic_super_app_chat_branches (id, conversation_id, label, created_at_unix_ms) VALUES (?, ?, 'Main', ?)")
    ...
    ) -> Result<ChatSidebarPage, AgenticSuperAppChatStoreError> {
        ...
        let rows = sqlx::query(
            "SELECT c.id, c.title, c.active_branch_id, c.pinned, c.archived, c.updated_at_unix_ms,
                (SELECT substr(json_extract(p.payload_json, '$.text'), 1, 160)
                 FROM agentic_super_app_chat_messages m
                 JOIN agentic_super_app_chat_message_parts p ON p.message_id=m.id AND p.kind='text'
                 WHERE m.conversation_id=c.id ORDER BY m.created_at_unix_ms DESC LIMIT 1) AS preview
             FROM agentic_super_app_chat_conversations c
             WHERE c.archived=? AND (c.title LIKE ? ESCAPE '\\' OR EXISTS (
                 SELECT 1 FROM agentic_super_app_chat_messages sm
                 JOIN agentic_super_app_chat_message_parts sp ON sp.message_id=sm.id AND sp.kind='text'
    ...
    pub async fn detail(
        ...
        let conversation = sqlx::query("SELECT id, title, active_branch_id, pinned, archived, created_at_unix_ms, updated_at_unix_ms FROM agentic_super_app_chat_conversations WHERE id=?")
            .bind(conversation_id).fetch_optional(self.persistence.pool()).await?
            .ok_or(AgenticSuperAppChatStoreError::NotFound)?;
        let branches = sqlx::query("SELECT b.id, b.parent_branch_id, b.forked_after_message_id, b.label, b.created_at_unix_ms, c.active_branch_id FROM agentic_super_app_chat_branches b JOIN agentic_super_app_chat_conversations c ON c.id=b.conversation_id WHERE b.conversation_id=? ORDER BY b.created_at_unix_ms")
        ...
        let turns = sqlx::query("SELECT id, message_id, assistant_message_id, branch_id, provider_account_id, model, reasoning_effort, state, job_id, input_tokens, output_tokens, created_at_unix_ms, updated_at_unix_ms FROM agentic_super_app_chat_turns WHERE conversation_id=? ORDER BY created_at_unix_ms")
            .bind(conversation_id).fetch_all(self.persistence.pool()).await?
            .into_iter().filter_map(|row| turn_from_row(row).ok()).collect();
        let draft =
            sqlx::query("SELECT draft FROM agentic_super_app_chat_drafts WHERE conversation_id=?")
                .bind(conversation_id)
                .fetch_optional(self.persistence.pool())
                .await?
                .map(|row| row.get(0))
                .unwrap_or_default();
        let event_cursor: i64 = sqlx::query_scalar("SELECT COALESCE(MAX(global_sequence), 0) FROM agentic_super_app_chat_events WHERE conversation_id=?")
    ...
    async fn messages(
        ...
        conversation_id: &str,
        branch_id: &str,
    ) -> Result<Vec<ChatMessage>, AgenticSuperAppChatStoreError> {
        let rows = sqlx::query("SELECT id, branch_id, role, turn_id, created_at_unix_ms FROM agentic_super_app_chat_messages WHERE conversation_id=? AND branch_id=? ORDER BY branch_position")
            .bind(conversation_id).bind(branch_id).fetch_all(self.persistence.pool()).await?;
        let mut result = Vec::with_capacity(rows.len());
        for row in rows {
            let id: String = row.get(0);
            let parts = sqlx::query("SELECT kind, payload_json FROM agentic_super_app_chat_message_parts WHERE message_id=? ORDER BY ordinal")
    ...
    ) -> Result<(), AgenticSuperAppChatStoreError> {
        self.require_conversation(&request.conversation_id).await?;
        sqlx::query("INSERT INTO agentic_super_app_chat_drafts (conversation_id, draft, updated_at_unix_ms) VALUES (?, ?, ?) ON CONFLICT(conversation_id) DO UPDATE SET draft=excluded.draft, updated_at_unix_ms=excluded.updated_at_unix_ms")
    ...
    pub async fn update_metadata(
        ...
        let current = sqlx::query(
            "SELECT title, pinned, archived FROM agentic_super_app_chat_conversations WHERE id=?",
        )
        ...
        sqlx::query("UPDATE agentic_super_app_chat_conversations SET title=?, pinned=?, archived=?, updated_at_unix_ms=? WHERE id=?")
    ...
    pub async fn register_attachment(
        ...
        relative_path: &str,
    ) -> Result<ChatAttachmentSummary, AgenticSuperAppChatStoreError> {
        sqlx::query("INSERT OR IGNORE INTO agentic_super_app_chat_attachments (id, display_name, mime_type, byte_count, sha256, relative_path, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?)")
        ...
        Ok(sqlx::query("SELECT id, display_name, mime_type, byte_count, sha256 FROM agentic_super_app_chat_attachments WHERE sha256=?")
    ...
    pub async fn attach_to_message(
        ...
        let message_exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM agentic_super_app_chat_messages WHERE id=? AND conversation_id=?)")
            .bind(message_id).bind(conversation_id).fetch_one(self.persistence.pool()).await?;
        if !message_exists {
            return Err(AgenticSuperAppChatStoreError::NotFound);
        }
        for attachment_id in attachment_ids {
            let attachment = sqlx::query("SELECT display_name, mime_type, byte_count, sha256 FROM agentic_super_app_chat_attachments WHERE id=?")
                .bind(attachment_id).fetch_optional(self.persistence.pool()).await?
                .ok_or(AgenticSuperAppChatStoreError::NotFound)?;
            let inserted = sqlx::query("INSERT OR IGNORE INTO agentic_super_app_chat_message_attachments (message_id, attachment_id) VALUES (?, ?)")
                .bind(message_id).bind(attachment_id).execute(self.persistence.pool()).await?;
            if inserted.rows_affected() > 0 {
                let ordinal: i64 = sqlx::query_scalar("SELECT COALESCE(MAX(ordinal), -1) + 1 FROM agentic_super_app_chat_message_parts WHERE message_id=?").bind(message_id).fetch_one(self.persistence.pool()).await?;
                ...
                sqlx::query("INSERT INTO agentic_super_app_chat_message_parts (message_id, ordinal, kind, payload_json) VALUES (?, ?, ?, ?)").bind(message_id).bind(ordinal).bind(kind).bind(serde_json::to_string(&summary)?).execute(self.persistence.pool()).await?;
            }
        }
        Ok(())
    }
    ...
    pub async fn remove_attachment(
        ...
        message_id: &str,
        attachment_id: &str,
    ) -> Result<Option<String>, AgenticSuperAppChatStoreError> {
        let mut tx = self.persistence.pool().begin().await?;
        let relative_path: Option<String> = sqlx::query_scalar("SELECT a.relative_path FROM agentic_super_app_chat_attachments a JOIN agentic_super_app_chat_message_attachments ma ON ma.attachment_id=a.id JOIN agentic_super_app_chat_messages m ON m.id=ma.message_id WHERE m.conversation_id=? AND m.id=? AND a.id=?")
            .bind(conversation_id).bind(message_id).bind(attachment_id).fetch_optional(&mut *tx).await?;
        let Some(relative_path) = relative_path else {
            return Err(AgenticSuperAppChatStoreError::NotFound);
        };
        sqlx::query("DELETE FROM agentic_super_app_chat_message_attachments WHERE message_id=? AND attachment_id=?").bind(message_id).bind(attachment_id).execute(&mut *tx).await?;
        sqlx::query("DELETE FROM agentic_super_app_chat_message_parts WHERE message_id=? AND kind IN ('attachment','image') AND payload_json LIKE ?").bind(message_id).bind(format!("%\\\"id\\\":\\\"{attachment_id}\\\"%" )).execute(&mut *tx).await?;
        let remaining: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM agentic_super_app_chat_message_attachments WHERE attachment_id=?",
        )
        .bind(attachment_id)
        .fetch_one(&mut *tx)
        .await?;
        if remaining == 0 {
            sqlx::query("DELETE FROM agentic_super_app_chat_attachments WHERE id=?")
                .bind(attachment_id)
                .execute(&mut *tx)
                .await?;
        }
    ...
    pub async fn start_turn(
        ...
        job_id: Option<&str>,
        command_request_id: Option<&str>,
    ) -> Result<AgenticSuperAppChatTurnStart, AgenticSuperAppChatStoreError> {
        if request.text.trim().is_empty() && request.attachment_ids.is_empty() {
            return Err(AgenticSuperAppChatStoreError::InvalidInput(
    ...
    ) -> Result<Option<AgenticSuperAppChatTurnStart>, AgenticSuperAppChatStoreError> {
        Ok(sqlx::query("SELECT conversation_id, branch_id, id, message_id, assistant_message_id, job_id FROM agentic_super_app_chat_turns WHERE command_request_id=?")
            .bind(command_request_id)
            .fetch_optional(self.persistence.pool())
            .await?
            .map(|row| AgenticSuperAppChatTurnStart {
    ...
    async fn start_turn_internal(
        ...
        let mut tx = self.persistence.pool().begin().await?;
        if let Some(command_request_id) = command_request_id {
            if let Some(row) = sqlx::query("SELECT conversation_id, branch_id, id, message_id, assistant_message_id, job_id FROM agentic_super_app_chat_turns WHERE command_request_id=?")
                .bind(command_request_id)
                .fetch_optional(&mut *tx)
                .await?
            {
                tx.rollback().await?;
                return Ok(AgenticSuperAppChatTurnStart {
                    conversation_id: row.get(0),
                    branch_id: row.get(1),
                    turn_id: row.get(2),
                    user_message_id: row.get(3),
        ...
        let conversation_exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM agentic_super_app_chat_conversations WHERE id=? AND active_branch_id=?)")
            .bind(&request.conversation_id).bind(&request.branch_id).fetch_one(&mut *tx).await?;
        if !conversation_exists {
            return Err(AgenticSuperAppChatStoreError::NotFound);
        }
        let active: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM agentic_super_app_chat_turns WHERE conversation_id=? AND state IN ('queued','streaming','cancel_requested')")
            .bind(&request.conversation_id).fetch_one(&mut *tx).await?;
        if active > 0 {
            return Err(AgenticSuperAppChatStoreError::ActiveTurn);
        }
        let now = now_ms();
        let (user_message_id, user_position) = if let Some(id) = existing_user_message_id {
            let row = sqlx::query("SELECT id, branch_position FROM agentic_super_app_chat_messages WHERE id=? AND conversation_id=? AND branch_id=? AND role='user'")
                .bind(id).bind(&request.conversation_id).bind(&request.branch_id).fetch_optional(&mut *tx).await?
                .ok_or(AgenticSuperAppChatStoreError::NotFound)?;
            (row.get(0), row.get(1))
        } else {
            let position: i64 = sqlx::query_scalar("SELECT COALESCE(MAX(branch_position), -1) + 1 FROM agentic_super_app_chat_messages WHERE branch_id=?")
                .bind(&request.branch_id).fetch_one(&mut *tx).await?;
            let id = Uuid::now_v7().to_string();
            sqlx::query("INSERT INTO agentic_super_app_chat_messages (id, conversation_id, branch_id, role, status, branch_position, created_at_unix_ms) VALUES (?, ?, ?, 'user', 'complete', ?, ?)")
            ...
            for attachment_id in &request.attachment_ids {
                let attachment = sqlx::query("SELECT display_name, mime_type, byte_count, sha256 FROM agentic_super_app_chat_attachments WHERE id=?")
                    ...
                    .ok_or(AgenticSuperAppChatStoreError::NotFound)?;
                sqlx::query("INSERT INTO agentic_super_app_chat_message_attachments (message_id, attachment_id) VALUES (?, ?)")
        ...
        sqlx::query("INSERT INTO agentic_super_app_chat_messages (id, conversation_id, branch_id, role, status, branch_position, turn_id, created_at_unix_ms) VALUES (?, ?, ?, 'assistant', 'streaming', ?, ?, ?)")
        ...
        sqlx::query("UPDATE agentic_super_app_chat_messages SET turn_id=? WHERE id=?")
            .bind(&turn_id)
            .bind(&assistant_message_id)
            .execute(&mut *tx)
            .await?;
        sqlx::query("INSERT INTO agentic_super_app_chat_turns (id, conversation_id, branch_id, message_id, assistant_message_id, provider_account_id, model, reasoning_effort, state, job_id, command_request_id, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'queued', ?, ?, ?, ?)")
        ...
        sqlx::query(
            "UPDATE agentic_super_app_chat_conversations SET updated_at_unix_ms=? WHERE id=?",
        )
        .bind(now)
        .bind(&request.conversation_id)
        .execute(&mut *tx)
        .await?;
        tx.commit().await?;
        Ok(AgenticSuperAppChatTurnStart {
    ...
    pub async fn apply_provider_event(
    ...
    ) -> Result<Option<ChatEventEnvelope>, AgenticSuperAppChatStoreError> {
        let mut tx = self.persistence.pool().begin().await?;
        let turn = sqlx::query("SELECT branch_id, assistant_message_id, state FROM agentic_super_app_chat_turns WHERE id=? AND conversation_id=?")
            .bind(turn_id).bind(conversation_id).fetch_optional(&mut *tx).await?
            .ok_or(AgenticSuperAppChatStoreError::NotFound)?;
        if event.provider_sequence >= 0 {
            let already_seen: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM agentic_super_app_chat_events WHERE turn_id=? AND provider_sequence_start=?")
        ...
        let (kind, payload) = match event.kind {
            ChatProviderStreamEventKind::TextDelta => {
                ...
                sqlx::query("UPDATE agentic_super_app_chat_turns SET state='streaming', updated_at_unix_ms=? WHERE id=?").bind(now_ms()).bind(turn_id).execute(&mut *tx).await?;
                sqlx::query(
                    "UPDATE agentic_super_app_chat_messages SET status='streaming' WHERE id=?",
                )
                .bind(&assistant_message_id)
                .execute(&mut *tx)
                .await?;
            ...
            ChatProviderStreamEventKind::ReasoningSummary => {
                ...
                let ordinal: i64 = sqlx::query_scalar("SELECT COALESCE(MAX(ordinal), -1) + 1 FROM agentic_super_app_chat_message_parts WHERE message_id=?").bind(&assistant_message_id).fetch_one(&mut *tx).await?;
                insert_part(
                    &mut tx,
                    &assistant_message_id,
                    ordinal,
            ...
            ChatProviderStreamEventKind::Usage => {
                ...
                sqlx::query("UPDATE agentic_super_app_chat_turns SET input_tokens=?, output_tokens=?, updated_at_unix_ms=? WHERE id=?")
            ...
            ChatProviderStreamEventKind::Completed => {
                ...
                sqlx::query("UPDATE agentic_super_app_chat_turns SET state='completed', input_tokens=?, output_tokens=?, updated_at_unix_ms=? WHERE id=?")
                    .bind(event.input_tokens.map(|v| v as i64)).bind(event.output_tokens.map(|v| v as i64)).bind(now_ms()).bind(turn_id).execute(&mut *tx).await?;
                sqlx::query(
                    "UPDATE agentic_super_app_chat_messages SET status='complete' WHERE id=?",
                )
                .bind(&assistant_message_id)
                .execute(&mut *tx)
                .await?;
            ...
            ChatProviderStreamEventKind::Failed => {
                ...
                sqlx::query("UPDATE agentic_super_app_chat_turns SET state='failed', updated_at_unix_ms=? WHERE id=?").bind(now_ms()).bind(turn_id).execute(&mut *tx).await?;
                sqlx::query(
                    "UPDATE agentic_super_app_chat_messages SET status='failed' WHERE id=?",
                )
                .bind(&assistant_message_id)
                .execute(&mut *tx)
                .await?;
    ...
    async fn set_turn_state(
        ...
        let mut tx = self.persistence.pool().begin().await?;
        let row = sqlx::query("SELECT branch_id, assistant_message_id, state FROM agentic_super_app_chat_turns WHERE id=? AND conversation_id=?").bind(turn_id).bind(conversation_id).fetch_optional(&mut *tx).await?.ok_or(AgenticSuperAppChatStoreError::NotFound)?;
        ...
        sqlx::query(
            "UPDATE agentic_super_app_chat_turns SET state=?, updated_at_unix_ms=? WHERE id=?",
        )
        .bind(state)
        .bind(now_ms())
        .bind(turn_id)
        .execute(&mut *tx)
        .await?;
        sqlx::query("UPDATE agentic_super_app_chat_messages SET status=? WHERE id=?")
            .bind(state)
    ...
    pub async fn retry_turn(
        ...
        let row = sqlx::query("SELECT branch_id, message_id FROM agentic_super_app_chat_turns WHERE id=? AND conversation_id=?").bind(turn_id).bind(conversation_id).fetch_optional(self.persistence.pool()).await?.ok_or(AgenticSuperAppChatStoreError::NotFound)?;
    ...
    pub async fn edit_message(
        &self,
        request: &agentic_super_app_protocol::ChatEditRequest,
        job_id: Option<&str>,
        command_request_id: Option<&str>,
    ) -> Result<AgenticSuperAppChatTurnStart, AgenticSuperAppChatStoreError> {
    ...
    async fn source_message(
        ...
        conversation_id: &str,
        message_id: &str,
    ) -> Result<(String, i64), AgenticSuperAppChatStoreError> {
        let row = sqlx::query("SELECT branch_id, branch_position FROM agentic_super_app_chat_messages WHERE id=? AND conversation_id=?").bind(message_id).bind(conversation_id).fetch_optional(self.persistence.pool()).await?.ok_or(AgenticSuperAppChatStoreError::NotFound)?;
    ...
    async fn copy_branch(
        ...
        label: &str,
        command_request_id: Option<&str>,
    ) -> Result<(String, String, String), AgenticSuperAppChatStoreError> {
        let mut tx = self.persistence.pool().begin().await?;
        if let Some(command_request_id) = command_request_id {
            if let Some(row) = sqlx::query(
                "SELECT id FROM agentic_super_app_chat_branches WHERE command_request_id=?",
            )
            .bind(command_request_id)
            .fetch_optional(&mut *tx)
            .await?
            {
                ...
                let copied_user_id = sqlx::query("SELECT id FROM agentic_super_app_chat_messages WHERE branch_id=? AND role='user' AND copied_from_message_id=? LIMIT 1")
                    .bind(&branch_id)
                    .bind(forked_after)
                    .fetch_optional(&mut *tx)
                    .await?
        ...
        sqlx::query("INSERT INTO agentic_super_app_chat_branches (id, conversation_id, parent_branch_id, forked_after_message_id, label, command_request_id, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?)")
        ...
        let source_messages = sqlx::query("SELECT id, role, status, branch_position, turn_id, created_at_unix_ms FROM agentic_super_app_chat_messages WHERE branch_id=? AND branch_position<=? ORDER BY branch_position")
        ...
        for row in source_messages {
            ...
            sqlx::query("INSERT INTO agentic_super_app_chat_messages (id, conversation_id, branch_id, copied_from_message_id, role, status, branch_position, turn_id, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
            ...
            let parts = sqlx::query("SELECT ordinal, kind, payload_json FROM agentic_super_app_chat_message_parts WHERE message_id=? ORDER BY ordinal").bind(&source_id).fetch_all(&mut *tx).await?;
            for part in parts {
                sqlx::query("INSERT INTO agentic_super_app_chat_message_parts (message_id, ordinal, kind, payload_json) VALUES (?, ?, ?, ?)").bind(&new_id).bind(part.get::<i64, _>(0)).bind(part.get::<String, _>(1)).bind(part.get::<String, _>(2)).execute(&mut *tx).await?;
            }
            let attachments = sqlx::query("SELECT attachment_id FROM agentic_super_app_chat_message_attachments WHERE message_id=?").bind(&source_id).fetch_all(&mut *tx).await?;
            for attachment in attachments {
                sqlx::query("INSERT INTO agentic_super_app_chat_message_attachments (message_id, attachment_id) VALUES (?, ?)").bind(&new_id).bind(attachment.get::<String, _>(0)).execute(&mut *tx).await?;
            }
        }
        sqlx::query("UPDATE agentic_super_app_chat_conversations SET active_branch_id=?, updated_at_unix_ms=? WHERE id=?").bind(&new_branch_id).bind(now_ms()).bind(conversation_id).execute(&mut *tx).await?;
        append_event(
            &mut tx,
            ChatEventInput {
                conversation_id,
    ...
    pub async fn events_since(
        ...
        after: i64,
        limit: u32,
    ) -> Result<Vec<ChatEventEnvelope>, AgenticSuperAppChatStoreError> {
        let rows = sqlx::query("SELECT global_sequence, aggregate_sequence, branch_id, turn_id, message_id, kind, payload_json, emitted_at_unix_ms FROM agentic_super_app_chat_events WHERE conversation_id=? AND global_sequence>? ORDER BY global_sequence LIMIT ?").bind(conversation_id).bind(after).bind(i64::from(limit.clamp(1, 500))).fetch_all(self.persistence.pool()).await?;
        rows.into_iter()
    ...
    pub async fn all_events_since(
        ...
        let rows = sqlx::query("SELECT global_sequence, aggregate_sequence, conversation_id, branch_id, turn_id, message_id, kind, payload_json, emitted_at_unix_ms FROM agentic_super_app_chat_events WHERE global_sequence>? ORDER BY global_sequence LIMIT 500").bind(after).fetch_all(self.persistence.pool()).await?;
        rows.into_iter().map(global_event_from_row).collect()
    }

    pub async fn interrupt_active_turns(&self) -> Result<usize, AgenticSuperAppChatStoreError> {
        let rows = sqlx::query("SELECT id, conversation_id, branch_id, assistant_message_id FROM agentic_super_app_chat_turns WHERE state IN ('queued','streaming','cancel_requested')")
        ...
        for row in rows {
            ...
            sqlx::query("UPDATE agentic_super_app_chat_turns SET state='interrupted', updated_at_unix_ms=? WHERE id=?")
                .bind(now_ms()).bind(&turn_id).execute(&mut *tx).await?;
            sqlx::query(
                "UPDATE agentic_super_app_chat_messages SET status='interrupted' WHERE id=?",
            )
            .bind(&assistant_message_id)
            .execute(&mut *tx)
            .await?;
    ...
    pub async fn attachments_for_branch(
        ...
        conversation_id: &str,
        branch_id: &str,
    ) -> Result<Vec<AgenticSuperAppChatStoredAttachment>, AgenticSuperAppChatStoreError> {
        let rows = sqlx::query("SELECT DISTINCT a.id, a.display_name, a.mime_type, a.byte_count, a.sha256, a.relative_path FROM agentic_super_app_chat_attachments a JOIN agentic_super_app_chat_message_attachments ma ON ma.attachment_id=a.id JOIN agentic_super_app_chat_messages m ON m.id=ma.message_id WHERE m.conversation_id=? AND m.branch_id=? ORDER BY a.created_at_unix_ms")
            .bind(conversation_id).bind(branch_id).fetch_all(self.persistence.pool()).await?;
    ...
    pub async fn delete_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<String>, AgenticSuperAppChatStoreError> {
        let paths: Vec<String> = sqlx::query("SELECT DISTINCT a.relative_path FROM agentic_super_app_chat_attachments a JOIN agentic_super_app_chat_message_attachments ma ON ma.attachment_id=a.id JOIN agentic_super_app_chat_messages m ON m.id=ma.message_id WHERE m.conversation_id=?").bind(conversation_id).fetch_all(self.persistence.pool()).await?.into_iter().map(|row| row.get(0)).collect();
        let result = sqlx::query("DELETE FROM agentic_super_app_chat_conversations WHERE id=?")
            .bind(conversation_id)
        ...
        for path in paths {
            let still_used: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM agentic_super_app_chat_attachments WHERE relative_path=?",
            )
            .bind(&path)
            ...
            if still_used == 0 {
                let _ = sqlx::query(
                    "DELETE FROM agentic_super_app_chat_attachments WHERE relative_path=?",
                )
                .bind(&path)
                .execute(self.persistence.pool())
                .await?;
    ...
    async fn require_conversation(&self, id: &str) -> Result<(), AgenticSuperAppChatStoreError> {
        let exists: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM agentic_super_app_chat_conversations WHERE id=? )",
        )
        .bind(id)
        .fetch_one(self.persistence.pool())
        .await?;
...
) -> Result<ChatEventEnvelope, AgenticSuperAppChatStoreError> {
    sqlx::query("UPDATE agentic_super_app_chat_conversations SET next_aggregate_sequence=next_aggregate_sequence+1 WHERE id=?")
        .bind(input.conversation_id).execute(&mut **tx).await?;
    let aggregate_sequence: i64 = sqlx::query_scalar(
        "SELECT next_aggregate_sequence FROM agentic_super_app_chat_conversations WHERE id=?",
    )
    .bind(input.conversation_id)
    .fetch_one(&mut **tx)
    .await?;
    let payload_json = serde_json::to_string(&input.payload)?;
    sqlx::query("INSERT INTO agentic_super_app_chat_events (conversation_id, aggregate_sequence, branch_id, turn_id, message_id, kind, payload_json, provider_sequence_start, provider_sequence_end, emitted_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
...
async fn insert_part(
    ...
    sqlx::query("INSERT INTO agentic_super_app_chat_message_parts (message_id, ordinal, kind, payload_json) VALUES (?, ?, ?, ?)").bind(message_id).bind(ordinal).bind(kind).bind(serde_json::to_string(&payload)?).execute(&mut **tx).await?;
    Ok(())
}
...
async fn append_text_part(
    ...
    if let Some(row) = sqlx::query("SELECT payload_json FROM agentic_super_app_chat_message_parts WHERE message_id=? AND kind='text' ORDER BY ordinal LIMIT 1").bind(message_id).fetch_optional(&mut **tx).await? {
        ...
        sqlx::query("UPDATE agentic_super_app_chat_message_parts SET payload_json=? WHERE message_id=? AND kind='text'").bind(serde_json::to_string(&payload)?).bind(message_id).execute(&mut **tx).await?;
    } else {
...
fn message_role(value: &str) -> Result<ChatMessageRole, AgenticSuperAppChatStoreError> {
    match value {
        "user" => Ok(ChatMessageRole::User),
        "assistant" => Ok(ChatMessageRole::Assistant),
        "system" => Ok(ChatMessageRole::System),
        _ => Err(AgenticSuperAppChatStoreError::Inconsistent),
    }
}
fn part_from_row(row: SqliteRow) -> Result<ChatMessagePart, AgenticSuperAppChatStoreError> {
    ...
    let attachment = || {
        serde_json::from_value(
            payload
                .get("attachment")
                .cloned()
                .ok_or(AgenticSuperAppChatStoreError::Inconsistent)?,
        )
        .map_err(AgenticSuperAppChatStoreError::from)
    };
    Ok(match kind.as_str() {
        ...
        _ => return Err(AgenticSuperAppChatStoreError::Inconsistent),
    })
}
fn turn_from_row(row: SqliteRow) -> Result<ChatTurnSummary, AgenticSuperAppChatStoreError> {
    Ok(ChatTurnSummary {
        id: row.get(0),
...
fn reasoning_from_value(value: &str) -> Result<ChatReasoningEffort, AgenticSuperAppChatStoreError> {
    match value {
...
fn turn_state_from_value(value: &str) -> Result<ChatTurnState, AgenticSuperAppChatStoreError> {
    match value {
...
fn stored_attachment_from_row(
    row: SqliteRow,
) -> Result<AgenticSuperAppChatStoredAttachment, AgenticSuperAppChatStoreError> {
    Ok(AgenticSuperAppChatStoredAttachment {
...
mod tests {
    ...
    fn temporary_database_path() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("agentic-super-app-chat-{}.sqlite3", Uuid::now_v7()))
    }
    ...
    async fn replayed_commands_and_provider_deltas_have_one_effect() {
        ...
        let persistence = AgenticSuperAppPersistence::open(&path)
            .await
            .expect("database");
        let store = AgenticSuperAppChatStore::new(persistence.clone());
        let created = store
            .create(
                &ChatCreateRequest {
