...
use agentic_super_app_model_gateway::{AgenticSuperAppModelProvider, AgenticSuperAppProviderError};
...
#[derive(Debug, Error)]
pub enum AgenticSuperAppAgentRuntimeError {
    #[error("agent store failure: {0}")]
    Store(#[from] AgenticSuperAppAgentStoreError),
...
#[derive(Clone)]
pub struct AgenticSuperAppAgentRuntime {
    store: AgenticSuperAppAgentStore,
...
impl AgenticSuperAppAgentRuntime {
    ...
    pub async fn initialize(&self) -> Result<(), AgenticSuperAppAgentRuntimeError> {
        for (id, source) in builtin_skill_sources() {
            let package = parse_skill_markdown(
                &format!("builtin/{id}/SKILL.md"),
                source,
                agentic_super_app_protocol::AgentSkillOrigin::Builtin,
            )
        ...
        for entry in entries.flatten().take(100) {
            ...
            if let Ok(package) = parse_skill_markdown(
                ...
                &source,
                agentic_super_app_protocol::AgentSkillOrigin::ApplicationData,
            ) {
                self.store.upsert_skill(&package).await?;
            }
        }
    ...
    pub async fn recover(&self) -> Result<usize, AgenticSuperAppAgentRuntimeError> {
        Ok(self.store.interrupt_active_runs().await?)
    }

    pub async fn dashboard(&self) -> Result<AgentDashboard, AgenticSuperAppAgentRuntimeError> {
        Ok(self.store.dashboard().await?)
    }

    pub async fn list_agents(
        &self,
    ) -> Result<Vec<agentic_super_app_protocol::AgentSummary>, AgenticSuperAppAgentRuntimeError>
    {
    ...
    pub async fn archive_agent(
        ...
        agent_id: &str,
        archived: bool,
    ) -> Result<(), AgenticSuperAppAgentRuntimeError> {
    ...
    ) -> Result<AgentFolderGrant, AgenticSuperAppAgentRuntimeError> {
    ...
    pub async fn skill_catalog(
        &self,
    ) -> Result<AgentSkillCatalog, AgenticSuperAppAgentRuntimeError> {
    ...
    ) -> Result<Vec<AgentMemorySummary>, AgenticSuperAppAgentRuntimeError> {
    ...
    ) -> Result<AgentMemorySummary, AgenticSuperAppAgentRuntimeError> {
    ...
    pub async fn conversations(
        ...
        Vec<agentic_super_app_protocol::AgentConversationSummary>,
        AgenticSuperAppAgentRuntimeError,
    > {
    ...
    ) -> Result<AgentConversationDetail, AgenticSuperAppAgentRuntimeError> {
    ...
    pub async fn run_detail(
        &self,
        run_id: &str,
    ) -> Result<AgentRunDetail, AgenticSuperAppAgentRuntimeError> {
        Ok(self.store.run_detail(run_id).await?)
    }

    pub async fn runs(
        &self,
        query: &agentic_super_app_protocol::AgentRunsQuery,
    ) -> Result<Vec<AgentRunSummary>, AgenticSuperAppAgentRuntimeError> {
        Ok(self
            .store
            .runs(
    ...
    ) -> Result<Vec<AgentEventEnvelope>, AgenticSuperAppAgentRuntimeError> {
    ...
    ) -> Result<AgentRunSummary, AgenticSuperAppAgentRuntimeError> {
    ...
    pub async fn resume_run(
        ...
        if !matches!(
            ...
            return Err(AgenticSuperAppAgentRuntimeError::InvalidInput(
    ...
    pub async fn decide_approval(
        ...
        let call = self
            ...
            .await?
            .ok_or_else(|| {
                AgenticSuperAppAgentRuntimeError::InvalidInput("tool call was not found".to_owned())
            })?;
        ...
        self.store.run(&approval.run_id).await?.ok_or_else(|| {
            AgenticSuperAppAgentRuntimeError::InvalidInput("run was not found".to_owned())
        })
    }
    ...
    pub async fn submit_input(
        ...
        let call_id = pending_call_id.ok_or_else(|| {
            AgenticSuperAppAgentRuntimeError::InvalidInput(
                "input request has no tool call".to_owned(),
            )
        })?;
        let call = self
            .store
            ...
            .ok_or_else(|| {
                AgenticSuperAppAgentRuntimeError::InvalidInput(
                    "input tool call was not found".to_owned(),
                )
            })?;
        ...
        self.spawn_run(run.id.clone());
        self.store.run(&run.id).await?.ok_or_else(|| {
            AgenticSuperAppAgentRuntimeError::InvalidInput("run was not found".to_owned())
        })
    }
    ...
    pub async fn export_agent(
        ...
        let manifest = json!({ "format": "agentic-super-app-agent", "version": 1, "agent": detail, "memory": memory }).to_string();
        self.artifacts
            .write_export(Path::new(&request.destination), &manifest, &[])
            .map_err(|error| AgenticSuperAppAgentRuntimeError::Artifact(error.to_string()))
    }
        ...
        tokio::spawn(async move {
            if let Err(error) = runtime.run_loop(&run_id, token).await {
                let should_mark_failed = !matches!(
                    error,
                    AgenticSuperAppAgentRuntimeError::Cancelled
                        | AgenticSuperAppAgentRuntimeError::Store(
                            AgenticSuperAppAgentStoreError::StaleLease
                        )
                );
    ...
    async fn run_loop(
        ...
        loop {
            ...
            let provider_secret = self
                ...
                .provider_secret_ref()
                .await
                .map_err(AgenticSuperAppAgentStoreError::Database)?
                .unwrap_or_default();
            ...
            if let Err(error) = provider_result {
                if matches!(error, AgenticSuperAppProviderError::Cancelled)
            ...
            let Some(call) = pending_call else {
                ...
                if matches!(
                    ...
                    agentic_super_app_protocol::AgentMemoryPolicy::IncludeSummaries
                ) {
                    let _ = self
                        .store
                        .upsert_memory(&AgentMemoryMutationRequest {
                            ...
                            memory_id: None,
                            class: agentic_super_app_protocol::AgentMemoryClass::RunSummary,
    ...
    ) -> Result<Option<PendingFunctionCall>, AgenticSuperAppAgentRuntimeError> {
        match event.kind {
    ...
    async fn process_tool_call(
        ...
        let fingerprint =
            agentic_super_app_approval_fingerprint(&call.name, &target, &call.arguments_json);
        ...
        match name {
            ...
            "memory.remember" => {
                ...
                let class = args
                    ...
                    .unwrap_or(agentic_super_app_protocol::AgentMemoryClass::AgentKnowledge);
                let memory = self
                    .store
                    .upsert_memory(&AgentMemoryMutationRequest {
                        agent_id: detail.summary.id.clone(),
    ...
    async fn retrieve_memory(
        ...
        if matches!(
            detail.memory_policy,
            agentic_super_app_protocol::AgentMemoryPolicy::Disabled
        ) {
            return Ok(());
        }
        let memories = self
...
mod tests {
    ...
    struct MockProvider;
    #[async_trait]
    impl AgenticSuperAppModelProvider for MockProvider {
        async fn validate_credentials(&self, _: &str) -> Result<(), AgenticSuperAppProviderError> {
            Ok(())
        }
        async fn stream_diagnostic(
            ...
            agentic_super_app_model_gateway::AgenticSuperAppProviderUsage,
