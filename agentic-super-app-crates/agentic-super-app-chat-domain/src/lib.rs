use agentic_super_app_protocol::{
    ChatMessage, ChatMessagePart, ChatReasoningEffort, ChatSendRequest,
};
use thiserror::Error;

#[derive(Debug, Error, PartialEq, Eq)]
pub enum AgenticSuperAppChatDomainError {
...
) -> Result<(), AgenticSuperAppChatDomainError> {
    if request.conversation_id.trim().is_empty() {
        return Err(AgenticSuperAppChatDomainError::MissingConversation);
    }
    if request.branch_id.trim().is_empty() {
        return Err(AgenticSuperAppChatDomainError::MissingBranch);
    }
    if request.provider_account_id.trim().is_empty() {
        return Err(AgenticSuperAppChatDomainError::MissingProvider);
    }
    if request.model.trim().is_empty() {
        return Err(AgenticSuperAppChatDomainError::MissingModel);
    }
    if request.text.trim().is_empty() && request.attachment_ids.is_empty() {
        return Err(AgenticSuperAppChatDomainError::EmptyTurn);
    }
    Ok(())
}
...
mod tests {
    use super::*;
    use agentic_super_app_protocol::{ChatAttachmentSummary, ChatMessagePart, ChatMessageRole};
    ...
    fn validation_rejects_empty_turns() {
        ...
        assert_eq!(
            ...
            Err(AgenticSuperAppChatDomainError::EmptyTurn)
        );
    }
