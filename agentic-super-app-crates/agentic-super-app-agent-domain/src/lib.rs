...
pub const AGENTIC_SUPER_APP_DEFAULT_MAX_STEPS: u32 = 24;
pub const AGENTIC_SUPER_APP_DEFAULT_MAX_TOOL_CALLS: u32 = 32;
pub const AGENTIC_SUPER_APP_DEFAULT_MAX_DURATION_SECONDS: u32 = 30 * 60;
pub const AGENTIC_SUPER_APP_DEFAULT_MAX_CONTEXT_TOKENS: u32 = 128_000;
pub const AGENTIC_SUPER_APP_DEFAULT_MAX_SUBAGENT_DEPTH: u8 = 2;
pub const AGENTIC_SUPER_APP_DEFAULT_MAX_CONCURRENT_SUBAGENTS: u8 = 2;
pub const AGENTIC_SUPER_APP_MAX_SKILL_FILE_BYTES: usize = 512 * 1024;
pub const AGENTIC_SUPER_APP_MAX_MEMORY_BYTES: usize = 32 * 1024;
pub const AGENTIC_SUPER_APP_MAX_MEMORY_RETRIEVALS: u32 = 8;

#[derive(Debug, Error, Clone, PartialEq, Eq)]
pub enum AgenticSuperAppAgentDomainError {
    #[error("{0} is required")]
    Required(&'static str),
...
pub fn default_runtime_limits() -> AgentRuntimeLimits {
    AgentRuntimeLimits {
        max_steps: AGENTIC_SUPER_APP_DEFAULT_MAX_STEPS,
        max_tool_calls: AGENTIC_SUPER_APP_DEFAULT_MAX_TOOL_CALLS,
        max_duration_seconds: AGENTIC_SUPER_APP_DEFAULT_MAX_DURATION_SECONDS,
        max_context_tokens: AGENTIC_SUPER_APP_DEFAULT_MAX_CONTEXT_TOKENS,
        max_subagent_depth: AGENTIC_SUPER_APP_DEFAULT_MAX_SUBAGENT_DEPTH,
        max_concurrent_subagents: AGENTIC_SUPER_APP_DEFAULT_MAX_CONCURRENT_SUBAGENTS,
    }
}

pub fn validate_agent_create(
    request: &AgentCreateRequest,
) -> Result<(), AgenticSuperAppAgentDomainError> {
...
pub fn validate_agent_update(
    ...
    if request.agent_id.trim().is_empty() {
        return Err(AgenticSuperAppAgentDomainError::Required("agent_id"));
    }
    validate_agent_fields(
        &request.name,
        &request.description,
...
fn validate_agent_fields(
    ...
    if name.trim().is_empty() {
        return Err(AgenticSuperAppAgentDomainError::Required("name"));
    }
    if name.chars().count() > 80 {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange("name"));
    }
    if description.chars().count() > 400 {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange("description"));
    }
    if operating_brief.chars().count() > 8_000 {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange(
            "operating_brief",
        ));
    }
    if avatar_color.trim().is_empty() || avatar_color.chars().count() > 32 {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange("avatar_color"));
    }
    if provider_account_id.trim().is_empty() {
        return Err(AgenticSuperAppAgentDomainError::Required(
            "provider_account_id",
        ));
    }
    if model.trim().is_empty() || model.chars().count() > 128 {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange("model"));
    }
    ...
    if limits.max_steps == 0 || limits.max_steps > 256 {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange("max_steps"));
    }
...
pub fn validate_run_transition(
    ...
    if valid {
        ...
        Err(AgenticSuperAppAgentDomainError::InvalidRunTransition { from, to })
    }
}
...
pub fn validate_tool_transition(
    ...
    if valid {
        ...
        Err(AgenticSuperAppAgentDomainError::InvalidToolTransition { from, to })
    }
}
...
pub fn memory_is_eligible_for_explicit_storage(
    ...
    if content.trim().is_empty() {
        return Err(AgenticSuperAppAgentDomainError::Required("content"));
    }
    if content.len() > AGENTIC_SUPER_APP_MAX_MEMORY_BYTES {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange(
            "memory content",
        ));
    }
    if looks_like_secret(content) {
        return Err(AgenticSuperAppAgentDomainError::SensitiveMemory);
    }
    Ok(())
}
...
pub fn parse_skill_markdown(
...
) -> Result<AgentSkillPackage, AgenticSuperAppAgentDomainError> {
    if source.len() > AGENTIC_SUPER_APP_MAX_SKILL_FILE_BYTES {
        return Err(AgenticSuperAppAgentDomainError::OutOfRange("skill file"));
    }
    let mut sections = source.splitn(3, "---");
    let first = sections.next().unwrap_or_default().trim();
    if !first.is_empty() {
        return Err(AgenticSuperAppAgentDomainError::InvalidSkill(
            "frontmatter must start at the beginning of the file".to_owned(),
        ));
    }
    let frontmatter = sections.next().ok_or_else(|| {
        AgenticSuperAppAgentDomainError::InvalidSkill("frontmatter is missing".to_owned())
    })?;
    let body = sections.next().ok_or_else(|| {
        AgenticSuperAppAgentDomainError::InvalidSkill("skill instructions are missing".to_owned())
    })?;
    ...
    for line in frontmatter.lines() {
        ...
        let (key, value) = line.split_once(':').ok_or_else(|| {
            AgenticSuperAppAgentDomainError::InvalidSkill(format!("invalid metadata line: {line}"))
        })?;
    ...
    let required = |key: &'static str| {
        values
            ...
            .ok_or(AgenticSuperAppAgentDomainError::Required(key))
    };
    let id = required("id")?;
    ...
    if triggers.is_empty() {
        return Err(AgenticSuperAppAgentDomainError::Required("triggers"));
    }
...
mod tests {
    use super::*;
    use agentic_super_app_protocol::{
        AgentApprovalPolicy, AgentRunState, AgentToolCallState, AgentToolRisk,
    };
