...
#[derive(Debug, Error)]
pub enum AgenticSuperAppWorkspaceError {
...
pub struct AgenticSuperAppWorkspaceService {
    workspaces: Arc<RwLock<HashMap<String, WorkspaceHandle>>>,
}

impl AgenticSuperAppWorkspaceService {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn open_workspace(
        ...
        let canonical_root = std::fs::canonicalize(path)
            .map_err(|error| AgenticSuperAppWorkspaceError::InvalidRoot(error.to_string()))?;
        let metadata = std::fs::metadata(&canonical_root)
            .map_err(|error| AgenticSuperAppWorkspaceError::InvalidRoot(error.to_string()))?;
        if !metadata.is_dir() {
            return Err(AgenticSuperAppWorkspaceError::InvalidRoot(
        ...
        self.workspaces
            .write()
            .map_err(|_| {
                AgenticSuperAppWorkspaceError::InvalidRoot("workspace lock poisoned".to_owned())
            })?
            .insert(
                id,
                WorkspaceHandle {
    ...
    pub fn summaries(&self) -> Result<Vec<CodeWorkspaceSummary>, AgenticSuperAppWorkspaceError> {
    ...
    pub fn require(
        ...
        if !allows(summary.trust, capability) {
            return Err(AgenticSuperAppWorkspaceError::CapabilityDenied(capability));
        }
        Ok(summary)
    }
    ...
    ) -> Result<CodeFileTree, AgenticSuperAppWorkspaceError> {
    ...
    pub fn read_file(
        ...
        workspace_id: &str,
        relative_path: &str,
    ) -> Result<CodeDocument, AgenticSuperAppWorkspaceError> {
        self.require(workspace_id, CodeWorkspaceCapability::ReadFiles)?;
        ...
        if metadata.file_type().is_symlink() {
            return Err(AgenticSuperAppWorkspaceError::SymlinkNotAllowed);
        }
        if !metadata.is_file() {
            return Err(AgenticSuperAppWorkspaceError::InvalidPath(
                "only regular files can be opened in the editor".to_owned(),
            ));
        }
        if metadata.len() > CODE_MAX_FILE_BYTES {
            return Err(AgenticSuperAppWorkspaceError::FileTooLarge);
        }
    ...
    pub fn save_file(
        ...
        if let Some(expected) = expected_fingerprint {
            ...
            if actual.as_deref() != Some(expected) {
                return Err(AgenticSuperAppWorkspaceError::FileConflict);
            }
        }

        let temporary_name = format!(".agentic-super-app-{}.tmp", Uuid::now_v7());
    ...
    fn handle(&self, workspace_id: &str) -> Result<WorkspaceHandle, AgenticSuperAppWorkspaceError> {
        self.workspaces
            .read()
            .map_err(|_| AgenticSuperAppWorkspaceError::NotFound)?
            .get(workspace_id)
...
fn open_directory(root: &Dir, relative: &str) -> Result<Dir, AgenticSuperAppWorkspaceError> {
...
fn open_parent_directory(
    root: &Dir,
    relative: &str,
) -> Result<(Dir, String), AgenticSuperAppWorkspaceError> {
    let mut components = relative.split('/').collect::<Vec<_>>();
    let file_name = components
        .pop()
        .ok_or(AgenticSuperAppWorkspaceError::InvalidPath(
...
fn document_from_bytes(
    ...
    let content = if binary {
        ...
        String::from_utf8(bytes.clone()).map_err(|_| AgenticSuperAppWorkspaceError::BinaryFile)?
    };
    Ok(CodeDocument {
        workspace_id: workspace_id.to_owned(),
...
fn domain_path_error(error: CodeDomainError) -> AgenticSuperAppWorkspaceError {
    AgenticSuperAppWorkspaceError::InvalidPath(error.to_string())
}
...
mod tests {
    ...
    fn reads_and_atomically_saves_inside_a_workspace() {
        let root = std::env::temp_dir().join(format!("agentic-code-workspace-{}", Uuid::now_v7()));
        fs::create_dir_all(root.join("src")).unwrap();
        fs::write(root.join("src/main.rs"), "fn main() {}\n").unwrap();
        let service = AgenticSuperAppWorkspaceService::new();
        let summary = service
    ...
    fn untrusted_workspace_cannot_save() {
        let root = std::env::temp_dir().join(format!("agentic-code-untrusted-{}", Uuid::now_v7()));
        ...
        assert!(matches!(
            ...
            Err(AgenticSuperAppWorkspaceError::CapabilityDenied(
