...
const tauriConfig = resolve(projectRoot, 'agentic-super-app-desktop', 'src-tauri', 'tauri.conf.json')
const packageManager = process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm'
const buildDir = mkdtempSync(join(tmpdir(), 'agentic-super-app-build-'))
...
try {
  ...
  const sourceExe = requiredFile(resolve(buildDir, 'release', 'agentic-super-app-desktop.exe'), 'Portable executable')
  ...
  const published = [
    ['Agentic-Super-App.msi', msi],
    ['Agentic-Super-App-setup.exe', setup],
    ['Agentic-Super-App-portable.exe', sourceExe],
  ]
