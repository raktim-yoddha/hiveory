...
import { CodePreviewPane } from '../code-workspace/panes/CodePreviewPane'
import '../code-workspace/code-workspace.css'
...
export function HiveoryCode() {
  ...
  const setWorkspaceTrust = async (grant: boolean) => {
    ...
    try {
      ...
      else setFeedback('Workspace trusted. File saves, terminals, Git reads, and the Browser are enabled.')
    } catch (error) {
  ...
  const openPreview = async () => {
    ...
    try {
      ...
      setFeedback('Browser opened in the docked pane. Local URLs and normal web pages are supported.')
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : 'The Browser address was blocked.')
    } finally {
      setBusy(null)
    }
  }
  ...
  return <section className="hiveory-code" aria-labelledby="hiveory-code-title">
    ...
    <div className="hiveory-code-grid">
      ...
      <section className="hiveory-code-main">
        {!selectedWorkspace ? <div className="hiveory-code-welcome"><div className="hiveory-empty-mark"><LayoutPanelTop size={28} /></div><h2>Choose a workspace</h2><p>Folders open in read-only mode first. Trust is an explicit per-workspace decision that unlocks saves, terminals, Git reads, and the Browser.</p><button onClick={() => void openWorkspace()}><FolderOpen size={15} />Open workspace folder</button></div> : <>
          ...
          {!trusted && <div className="hiveory-code-trust-banner" role="status"><ShieldAlert size={17} /><div><strong>This workspace is untrusted</strong><p>Reading and listing are available. Trusting enables file saves, process execution, Git status/diff, and the Browser.</p></div><button onClick={() => void setWorkspaceTrust(true)} disabled={busy !== null}>Trust workspace</button></div>}
