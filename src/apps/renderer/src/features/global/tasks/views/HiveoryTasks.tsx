import { CircleAlert, ExternalLink, Filter, LoaderCircle, Plus, RefreshCw, Settings2, X } from 'lucide-react'
...
import { HiveoryButton, HiveoryEmptyState, HiveoryIconButton, HiveoryPageHeader, HiveorySearchField, HiveorySelect, HiveoryTabs, useHiveoryDialogs } from '../../../../shared/ui/HiveoryDesign'
...
export function HiveoryTasks({ onOpenWorkspace, onStartLocalWork }: { onOpenWorkspace: (workspaceId: string) => void; onStartLocalWork: () => void }) {
  ...
  const remove = async (sourceId: string) => { if (!workspaceId || !await confirm({ title: 'Remove task source?', description: 'This removes the local task-source connection and its stored credentials. This cannot be undone.', confirmLabel: 'Remove source', intent: 'danger' })) return; setBusy(true); try { await hiveoryClient.removeTaskSource(workspaceId, sourceId); await load(workspaceId) } catch (cause) { setError(cause instanceof Error ? cause.message : 'The source could not be removed.') } finally { setBusy(false) } }
  ...
  return <section className="hiveory-page hiveory-tasks-page hiveory-task-sources" aria-labelledby="hiveory-tasks-title">
    ...
    {!workspaceId && !loading ? <HiveoryEmptyState title="Open a workspace first" action={<HiveoryButton type="button" intent="primary" onClick={onStartLocalWork}>Open workspace</HiveoryButton>}><><Settings2 size={22} aria-hidden="true" />Task sources are scoped to a local workspace so GitHub, Jira, and Linear never bleed into another project.</></HiveoryEmptyState> : <>
      <div className="hiveory-task-source-strip">{sourceKinds.map((kind) => { const source = snapshot?.sources.find((item) => item.provider === kind); return <div key={kind} className="hiveory-task-source-chip"><span><HiveoryBrandIcon provider={kind} size={15} /></span><div><strong>{providerName[kind]}</strong><small>{source?.validated_at_unix_ms ? 'Connected' : source?.last_error ?? (kind === 'github' ? 'Local CLI' : 'Not connected')}</small></div>{kind !== 'github' && <button type="button" onClick={() => setConnector(kind)}>{configured.has(kind) ? <Settings2 size={14} /> : <Plus size={14} />}</button>}{source && kind !== 'github' && <button type="button" className="hiveory-task-source-remove" onClick={() => void remove(source.id)} disabled={busy} aria-label={`Remove ${providerName[kind]}`}><X size={13} /></button>}</div> })}</div>
