...
function CapabilityPageHeader({ id, title, subtitle }: { id: string; title: string; subtitle: string }) {
  return <HiveoryPageHeader id={id} title={title} subtitle={subtitle} className="hiveory-capability-page-header" />
}

function SettingsButton({ className, intent = 'secondary', size = 'standard', ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { intent?: 'primary' | 'secondary' | 'ghost' | 'danger'; size?: 'standard' | 'compact' | 'icon' }) {
  return <HiveoryButton {...props} intent={intent} size={size} className={['hiveory-settings-button', className].filter(Boolean).join(' ')} />
}
...
export function HiveoryCapabilitySettings({
  ...
  return <div className="hiveory-capability-settings">
    <aside className="hiveory-capability-sidebar" aria-label="Settings navigation">
      <SettingsButton type="button" intent="ghost" className="hiveory-capability-back" onClick={onBackToApp}><ArrowLeft size={16} />Back to app</SettingsButton>
...
function SettingsNavButton({ item, section, onSelect }: { item: { id: SettingsSection; label: string; icon: typeof ListChecks }; section: SettingsSection; onSelect: (value: SettingsSection) => void }) {
  ...
  return <SettingsButton type="button" className={section === item.id ? 'is-selected' : ''} onClick={() => onSelect(item.id)}><Icon size={16} />{item.label}</SettingsButton>
}
...
function IntegrationsPanel() {
  ...
  return <section className="hiveory-capability-page" aria-labelledby="hiveory-integrations-title">
    <CapabilityPageHeader id="hiveory-integrations-title" title="Integrations" subtitle="See which modes are available in this edition." />
    <section className="hiveory-capability-surface">
      ...
      <SettingsRow title="Agent mode integration" detail={isHiveoryDev ? 'Ready to use in Dev.' : 'Locked in Production. Activation will be available when premium services launch.'}>
        <SettingsButton type="button" disabled={!isHiveoryDev} onClick={openAgentMode}>{isHiveoryDev ? 'Open Agent mode' : 'Unavailable'}</SettingsButton>
      </SettingsRow>
    </section>
  </section>
}
...
function OnboardingPanel({ adapters, detectedAdapters, plugins, connectedPluginIds, snapshot, preferences, steps, selectedStep, busy, orchestrationInstalled, computerUseInstalled, browserUseInstalled, completedSetup, onSelectStep, onConfirmNotifications, onSelectAgent, onInstallSkills, onRefresh, onOpenWorkbench, onAddProject, onSaveSetup, onUpdatePreferences }: {
  ...
  return <section className="hiveory-capability-page hiveory-onboarding" aria-labelledby="hiveory-onboarding-title">
    <CapabilityPageHeader id="hiveory-onboarding-title" title="Onboarding checklist" subtitle="Finish the core workflows that make Hiveory useful for parallel local agent work." />
    <div className="hiveory-onboarding-layout">
      <nav className="hiveory-onboarding-steps" aria-label="Onboarding steps">
        ...
        {steps.slice(0, 6).map((step, index) => <SettingsButton type="button" key={step.id} className={selectedStep === step.id ? 'is-selected' : ''} onClick={() => onSelectStep(step.id)}><span className={step.done ? 'is-done' : ''}>{step.done ? <Check size={13} /> : index + 1}</span>{step.label}</SettingsButton>)}
        ...
        {steps.slice(6).map((step) => <SettingsButton type="button" key={step.id} className={selectedStep === step.id ? 'is-selected' : ''} onClick={() => onSelectStep(step.id)}><span className={step.done ? 'is-done' : ''}>{step.done ? <Check size={13} /> : <Circle size={12} />}</span>{step.label}</SettingsButton>)}
      </nav>
      <article className="hiveory-onboarding-detail">
        ...
        {selectedStep === 'notifications' && <><h2>Turn on notifications</h2><p>Know when an agent finishes, needs attention, or is blocked. Hiveory sends the test through the native desktop notification channel.</p><div className="hiveory-capability-form-block"><strong>Notification delivery</strong><span>Windows native notification</span><SettingsButton type="button" intent="primary" onClick={onConfirmNotifications} disabled={busy !== null}><Bell size={15} />{busy === 'notifications' ? 'Sending…' : 'Send test notification'}</SettingsButton></div></>}
        {selectedStep === 'agent' && <><h2>Choose your default agent</h2><p>Start a pane faster with a detected coding agent already selected. This choice is retained for the application.</p><p className="hiveory-capability-kicker"><span />Detected on this system · {detectedAdapters.length}</p><div className="hiveory-agent-choice-grid">{adapters.map((adapter) => <SettingsButton type="button" key={adapter.id} disabled={!adapter.detected} className={preferences.defaultAdapterId === adapter.id ? 'is-selected' : ''} onClick={() => onSelectAgent(adapter.id)}><Terminal size={18} /><span><strong>{adapterLabel(adapter)}</strong><small>{adapter.detected ? adapter.executable : 'Not detected'}</small></span>{preferences.defaultAdapterId === adapter.id && <CheckCircle2 size={18} />}</SettingsButton>)}</div>{!adapters.length && <p className="hiveory-capability-empty">Refreshing the desktop host will discover installed coding agents.</p>}</>}
        {selectedStep === 'cli' && <><h2>Enable Hiveory CLI & skills</h2><p>Install the capability skills into the durable agent catalog. Adapter detection below comes from the local desktop host.</p><div className="hiveory-capability-card-grid"><CapabilityCard icon={<Workflow size={20} />} title="Agent Orchestration" detail="Coordinate durable work, worker dispatches, handoffs, and decision gates." state={orchestrationInstalled ? 'Installed' : 'Not installed'} /><CapabilityCard icon={<MonitorCog size={20} />} title="Computer Use" detail="Give Hiveory agents a clear local browser and desktop-operation skill." state={computerUseInstalled ? 'Installed' : 'Not installed'} /><CapabilityCard icon={<Globe2 size={20} />} title="Browser Use" detail="Drive an embedded Browser or hand work to an external browser with approvals." state={browserUseInstalled ? 'Installed' : 'Not installed'} /></div><div className="hiveory-capability-actions"><SettingsButton type="button" intent="primary" onClick={onInstallSkills} disabled={busy !== null}><Terminal size={15} />{busy?.startsWith('install-') ? 'Installing…' : 'Install CLI & skills'}</SettingsButton><SettingsButton type="button" className="is-secondary" onClick={onRefresh} disabled={busy !== null}><RefreshCw size={15} />Re-check adapters</SettingsButton></div><div className="hiveory-availability-list">{adapters.map((adapter) => <span key={adapter.id} className={adapter.detected ? 'is-ready' : ''}>{adapterLabel(adapter)}<small>{adapter.detected ? 'ready' : 'missing'}</small></span>)}</div></>}
        {selectedStep === 'integrations' && <><h2>Connect integrations</h2><p>Plugins and task sources are available to Hiveory agents only after their local connection is configured and validated.</p><div className="hiveory-integration-list">{plugins.length ? plugins.map((plugin) => <div key={plugin.manifest.id}><HiveoryBrandIcon provider={plugin.manifest.id} size={18} /><span><strong>{plugin.manifest.name}</strong><small>{plugin.manifest.description}</small></span><b className={connectedPluginIds.has(plugin.manifest.id) ? 'is-connected' : ''}>{connectedPluginIds.has(plugin.manifest.id) ? 'Connected' : 'Not connected'}</b></div>) : <p className="hiveory-capability-empty">No plugin catalog is available yet. Open Plugins in Workbench to add an integration.</p>}</div><SettingsButton type="button" onClick={onOpenWorkbench}><PlugZap size={15} />Open plugins in Workbench</SettingsButton></>}
        {selectedStep === 'setup' && <><h2>Automate workspace setup</h2><p>Save the install or preparation command Hiveory should run in the first terminal of each new trusted worktree for this project.</p><div className="hiveory-capability-form-block"><label>Project<select value={preferences.setupProjectId ?? ''} onChange={(event) => onUpdatePreferences({ setupProjectId: event.target.value || null })}><option value="">Choose a project</option>{snapshot?.projects.map((project) => <option key={project.id} value={project.id}>{project.display_name}</option>)}</select></label><label>Setup command<input value={preferences.setupCommand} placeholder="pnpm install" onChange={(event) => onUpdatePreferences({ setupCommand: event.target.value })} /></label><SettingsButton type="button" intent="primary" onClick={onSaveSetup} disabled={!preferences.setupProjectId || !preferences.setupCommand.trim()}><Check size={15} />Save & automate</SettingsButton></div><p className="hiveory-capability-note">The command is local to Hiveory and runs only after the destination worktree is trusted.</p></>}
        {selectedStep === 'projects' && <><h2>Start work in multiple repos</h2><p>Bring your repositories into Hiveory so you can open agent workspaces without hunting for folders.</p><SettingsButton type="button" intent="primary" onClick={onAddProject} disabled={busy !== null}><FolderPlus size={15} />Add project</SettingsButton><ProjectPreview projects={snapshot?.projects ?? []} /></>}
        {selectedStep === 'multi-task' && <><h2>Multi-task</h2><p>Work in multiple isolated workspaces at once. Hiveory tracks every local worktree and the agent sessions inside it.</p><WorkspacePreview count={snapshot?.workspaces.length ?? 0} /><SettingsButton type="button" onClick={onOpenWorkbench}><Workflow size={15} />Open Workbench</SettingsButton></>}
        {selectedStep === 'browser' && <><h2>Use Hiveory browser</h2><p>Let agents work in the embedded Browser, open the same page in another browser, or use the user-authorized desktop. Choose the target in Browser Use settings.</p><div className="hiveory-browser-illustration"><Globe2 size={22} /><span /><span /><i /></div><div className="hiveory-capability-actions"><SettingsButton type="button" onClick={onOpenWorkbench}><Globe2 size={15} />Open Browser pane</SettingsButton><SettingsButton type="button" className="is-secondary" onClick={() => onSelectStep('cli')}><Terminal size={15} />Open CLI &amp; skills</SettingsButton></div></>}
      </article>
    </div>
  </section>
}
...
function AgentsPanel({ adapters, detectedAdapters, preferences, busy, onSelect, onUpdate, onPermissionMode, onRefresh }: { adapters: CodeAdapterSummary[]; detectedAdapters: CodeAdapterSummary[]; preferences: CapabilityPreferences; busy: string | null; onSelect: (id: string) => void; onUpdate: (patch: Partial<CapabilityPreferences>) => void; onPermissionMode: (mode: CapabilityPreferences['permissionMode']) => void; onRefresh: () => void }) {
  return <section className="hiveory-capability-page" aria-labelledby="hiveory-agents-title"><CapabilityPageHeader id="hiveory-agents-title" title="Agents" subtitle="Manage coding agents, set a default, and control launch preferences." /><section className="hiveory-capability-surface"><h2>Default agent</h2><p>Default agent and launch preferences are retained locally. Hiveory validates installation on every launch.</p><div className="hiveory-agent-inline-list">{adapters.map((adapter) => <SettingsButton type="button" key={adapter.id} disabled={!adapter.detected} onClick={() => onSelect(adapter.id)} className={preferences.defaultAdapterId === adapter.id ? 'is-selected' : ''}><Terminal size={14} />{adapterLabel(adapter)}{preferences.defaultAdapterId === adapter.id && <Check size={14} />}</SettingsButton>)}</div><SettingsRow title="Agent runtime" detail="Choose the execution environment used for new coding panes."><div className="hiveory-segmented"><SettingsButton type="button" className={preferences.runtime === 'windows' ? 'is-selected' : ''} onClick={() => onUpdate({ runtime: 'windows' })}>Windows</SettingsButton><SettingsButton type="button" className={preferences.runtime === 'wsl' ? 'is-selected' : ''} onClick={() => onUpdate({ runtime: 'wsl' })}>WSL</SettingsButton></div></SettingsRow><SettingsRow title="Agent status hooks" detail="Show working, waiting, and completed states in Hiveory."><Toggle checked={preferences.agentStatusHooks} onChange={(checked) => onUpdate({ agentStatusHooks: checked })} label="Toggle agent status hooks" /></SettingsRow><SettingsRow title="Auto-generate pane titles" detail="Keep stable, short pane names until you rename a pane yourself."><Toggle checked={preferences.autoTabTitles} onChange={(checked) => onUpdate({ autoTabTitles: checked })} label="Toggle automatic pane titles" /></SettingsRow><SettingsRow title="Keep computer awake" detail="Control whether Hiveory requests an awake system while agents work."><div className="hiveory-segmented">{(['on', 'agent', 'off'] as const).map((value) => <SettingsButton type="button" key={value} className={preferences.computerAwake === value ? 'is-selected' : ''} onClick={() => onUpdate({ computerAwake: value })}>{value === 'agent' ? 'Agent' : value[0].toUpperCase() + value.slice(1)}</SettingsButton>)}</div></SettingsRow><SettingsRow title="Prompt cache timer" detail="Show retained-context timing in supporting CLI panes."><Toggle checked={preferences.cacheTimer} onChange={(checked) => onUpdate({ cacheTimer: checked })} label="Toggle prompt cache timer" /></SettingsRow><SettingsRow title="Agent permissions" detail="Choose the default permission behavior for supported coding-agent panes."><div className="hiveory-segmented"><SettingsButton type="button" className={preferences.permissionMode === 'yolo' ? 'is-selected' : ''} onClick={() => onPermissionMode('yolo')}>YOLO</SettingsButton><SettingsButton type="button" className={preferences.permissionMode === 'manual' ? 'is-selected' : ''} onClick={() => onPermissionMode('manual')}>Manual</SettingsButton></div></SettingsRow><div className="hiveory-capability-list-heading"><h2>Detected agents</h2><SettingsButton type="button" className="is-secondary" onClick={onRefresh} disabled={busy !== null}><RefreshCw size={14} />Refresh</SettingsButton></div><div className="hiveory-adapter-settings-list">{adapters.map((adapter) => <div key={adapter.id}><Terminal size={17} /><span><strong>{adapterLabel(adapter)}</strong><small><code>{adapter.executable}</code>{adapter.detected ? ' · detected' : ' · not detected'}</small></span><b className={adapter.detected ? 'is-ready' : ''}>{adapter.detected ? 'Available' : 'Unavailable'}</b>{adapter.detected && <SettingsButton type="button" onClick={() => onSelect(adapter.id)}>{preferences.defaultAdapterId === adapter.id ? 'Default' : 'Set default'}</SettingsButton>}</div>)}</div>{!detectedAdapters.length && <p className="hiveory-capability-empty">No installed coding agents were detected. Install a supported CLI, then refresh.</p>}</section></section>
}
...
function OrchestrationPanel({ installed, preferences, adapters, busy, onInstall, onRefresh, onUpdate, onOpenWorkbench }: { installed: boolean; preferences: CapabilityPreferences; adapters: CodeAdapterSummary[]; busy: string | null; onInstall: () => void; onRefresh: () => void; onUpdate: (patch: Partial<CapabilityPreferences>) => void; onOpenWorkbench: () => void }) {
  return <section className="hiveory-capability-page" aria-labelledby="hiveory-orchestration-title"><CapabilityPageHeader id="hiveory-orchestration-title" title="Orchestration" subtitle="Coordinate multiple coding agents through Hiveory’s durable runs." /><section className="hiveory-capability-surface"><div className="hiveory-skill-hero"><span><Workflow size={23} /></span><div><h2>Orchestration skill <b className={installed ? 'is-connected' : ''}>{installed ? 'Installed' : 'Not installed'}</b></h2><p>Enables agents to coordinate work through bounded runs, worker dispatches, durable mailbox delivery, and decision gates.</p><div><SettingsButton type="button" intent="primary" onClick={onInstall} disabled={busy !== null || installed}><Terminal size={15} />{installed ? 'Installed' : 'Install'}</SettingsButton><SettingsButton type="button" className="is-secondary" onClick={onRefresh} disabled={busy !== null}><RefreshCw size={15} />Re-check</SettingsButton></div></div></div><div className="hiveory-coverage"><h2>Agent coverage</h2><p>Detected adapters can coordinate work when a trusted workspace is available.</p><div>{adapters.length ? adapters.map((adapter) => <span key={adapter.id}><Terminal size={13} />{adapterLabel(adapter)}<small>ready</small></span>) : <p>No coding agents are detected yet.</p>}</div></div><SettingsRow title="Nested worker depth" detail="Choose how many generations of workers may spawn more workers. One keeps coordination flat."><input className="hiveory-depth-input" type="number" min="1" max="4" value={preferences.nestedWorkerDepth} onChange={(event) => onUpdate({ nestedWorkerDepth: Math.max(1, Math.min(4, Number(event.target.value) || 1)) })} aria-label="Nested worker depth" /></SettingsRow><div className="hiveory-how-to"><h2>How to use it</h2><p>Open a trusted workspace, then use Coordination to create a bounded run and choose a worker adapter.</p><div><CapabilityCard icon={<Workflow size={18} />} title="Hand off an active task" detail="Move structured context to a worker through the durable inbox." /><CapabilityCard icon={<GitBranch size={18} />} title="Hand off to another worktree" detail="Keep parallel work isolated in its own workspace." /><CapabilityCard icon={<ListChecks size={18} />} title="Run a phased workflow" detail="Build a task graph and dispatch each ready phase." /><CapabilityCard icon={<Network size={18} />} title="Run independent work in parallel" detail="Send non-overlapping tasks to multiple workers." /></div><SettingsButton type="button" onClick={onOpenWorkbench}><Workflow size={15} />Open Coordination</SettingsButton></div></section></section>
}
...
function BrowserUsePanel({ installed, busy, preferences, onInstall, onRefresh, onOpenWorkbench, onOpenExternal, onUpdate }: { installed: boolean; busy: string | null; preferences: CapabilityPreferences; onInstall: () => void; onRefresh: () => void; onOpenWorkbench: () => void; onOpenExternal: (url: string) => void; onUpdate: (patch: Partial<CapabilityPreferences>) => void }) {
  ...
  return <section className="hiveory-capability-page" aria-labelledby="hiveory-browser-use-title"><CapabilityPageHeader id="hiveory-browser-use-title" title="Browser Use" subtitle="Give agents a real browser surface and a controlled path to the rest of the user's computer." /><section className="hiveory-capability-surface"><div className="hiveory-skill-hero"><span><Globe2 size={23} /></span><div><h2>Browser Use skill <b className={installed ? 'is-connected' : ''}>{installed ? 'Installed' : 'Not installed'}</b></h2><p>Agents can inspect and operate the embedded Hiveory Browser, open a page in an external browser, or use the approved desktop accessibility surface.</p><div><SettingsButton type="button" intent="primary" onClick={onInstall} disabled={busy !== null || installed}><Terminal size={15} />{installed ? 'Installed' : 'Install'}</SettingsButton><SettingsButton type="button" className="is-secondary" onClick={onRefresh} disabled={busy !== null}><RefreshCw size={15} />Re-check</SettingsButton></div></div></div><SettingsRow title="Enable Agent Browser Use" detail="Expose browser.* and computer.* tools to agents and include browser target guidance in new runs."><Toggle checked={preferences.browserUseEnabled} onChange={(checked) => onUpdate({ browserUseEnabled: checked })} label="Enable Agent Browser Use" /></SettingsRow><SettingsRow title="Browser target" detail="Choose where browser work should happen for the next agent run."><div className="hiveory-segmented hiveory-browser-target-segment"><SettingsButton type="button" className={preferences.browserTarget === 'inner' ? 'is-selected' : ''} onClick={() => onUpdate({ browserTarget: 'inner' })}><Globe2 size={14} />Inner Browser</SettingsButton><SettingsButton type="button" className={preferences.browserTarget === 'external' ? 'is-selected' : ''} onClick={() => onUpdate({ browserTarget: 'external' })}><Globe2 size={14} />External browser</SettingsButton><SettingsButton type="button" className={preferences.browserTarget === 'desktop' ? 'is-selected' : ''} onClick={() => onUpdate({ browserTarget: 'desktop' })}><MonitorCog size={14} />User's PC</SettingsButton></div></SettingsRow><div className="hiveory-browser-use-target-card"><strong>{preferences.browserTarget === 'inner' ? 'Embedded Hiveory Browser' : preferences.browserTarget === 'external' ? 'External browser handoff' : 'Desktop computer use'}</strong><p>{preferences.browserTarget === 'inner' ? 'Agents use browser.snapshot, browser.state, browser.navigate, browser.click, browser.fill, browser.press, browser.scroll, and browser.capture against the active Browser pane.' : preferences.browserTarget === 'external' ? 'Agents may open a page directly with browser.open_external_url or hand off the active page with browser.open_external. Existing browser sessions remain outside Hiveory and are controlled through computer.list_apps, computer.snapshot, and approved computer.action calls.' : 'Agents use computer.capabilities, computer.list_apps, computer.list_windows, computer.snapshot, and computer.action for user-authorized browsers and local apps. Shell commands remain available through computer.run.'}</p>{preferences.browserTarget === 'inner' && <SettingsButton type="button" onClick={onOpenWorkbench}><Globe2 size={15} />Open Browser pane</SettingsButton>}{preferences.browserTarget === 'external' && <div className="hiveory-browser-external-form"><label>Page to open<input value={url} onChange={(event) => setUrl(event.target.value)} onBlur={saveUrl} placeholder="https://www.google.com/" /></label><SettingsButton type="button" intent="primary" onClick={() => { saveUrl(); onOpenExternal(url) }} disabled={!url.trim()}><Globe2 size={15} />Open external browser</SettingsButton></div>}{preferences.browserTarget === 'desktop' && <SettingsButton type="button" className="is-secondary" onClick={() => onUpdate({ browserTarget: 'desktop' })}><MonitorCog size={15} />Desktop target selected</SettingsButton>}</div><div className="hiveory-capability-note"><ShieldCheck size={17} /><span>Browser and desktop actions are visible in the Agent run timeline. Hiveory asks for approval according to the selected policy before external side effects.</span></div></section></section>
}
...
function ComputerUsePanel({ installed, busy, onInstall, onRefresh, onOpenWorkbench }: { installed: boolean; busy: string | null; onInstall: () => void; onRefresh: () => void; onOpenWorkbench: () => void }) {
  return <section className="hiveory-capability-page" aria-labelledby="hiveory-computer-use-title"><CapabilityPageHeader id="hiveory-computer-use-title" title="Computer Use" subtitle="Enable agents to work with local desktop and in-app browser surfaces." /><section className="hiveory-capability-surface"><div className="hiveory-skill-hero"><span><MonitorCog size={23} /></span><div><h2>Computer Use skill <b className={installed ? 'is-connected' : ''}>{installed ? 'Up to date' : 'Not installed'}</b></h2><p>Gives agents explicit instructions for local Browser panes and user-authorized desktop work.</p><div><SettingsButton type="button" intent="primary" onClick={onInstall} disabled={busy !== null || installed}><Terminal size={15} />{installed ? 'Installed' : 'Install'}</SettingsButton><SettingsButton type="button" className="is-secondary" onClick={onRefresh} disabled={busy !== null}><RefreshCw size={15} />Re-check</SettingsButton></div></div></div><div className="hiveory-computer-use-note"><ShieldCheck size={18} /><p>Hiveory keeps Browser panes inside the app and routes external mutations through the agent’s selected approval policy.</p></div><SettingsButton type="button" onClick={onOpenWorkbench}><Globe2 size={15} />Open Browser pane</SettingsButton></section></section>
}
