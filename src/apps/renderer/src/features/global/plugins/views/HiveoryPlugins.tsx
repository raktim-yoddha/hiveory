...
import { HiveoryButton, HiveoryDialog, HiveoryEmptyState, HiveoryIconButton, HiveoryMenu, HiveoryMenuItem, HiveoryPageHeader, HiveorySearchField, HiveoryTabs, useHiveoryDialogs, useHiveoryDismissibleLayer } from '../../../../shared/ui/HiveoryDesign'
...
function pluginMark(entry: PluginCatalogEntry) {
  return <span className="hiveory-plugin-mark"><HiveoryBrandIcon provider={entry.manifest.id} size={20} /></span>
}
...
export function HiveoryPlugins() {
  ...
  const deletePlugin = async (entry: PluginCatalogEntry) => {
    const accepted = await confirm({ title: `Delete ${entry.manifest.name}?`, description: 'This permanently removes the plugin, its connections, stored credentials, grants, and invocation history. This cannot be undone.', confirmLabel: 'Delete plugin', intent: 'danger' })
    if (!accepted) return
    setOpenMenuId(null)
    await action(`delete-plugin-${entry.manifest.id}`, () => hiveoryClient.deletePlugin(entry.manifest.id), `${entry.manifest.name} deleted.`)
  }
  ...
  return <section className="hiveory-page hiveory-capability-hub" aria-labelledby="hiveory-capability-title">
    <HiveoryPageHeader id="hiveory-capability-title" title="Plugins" subtitle="Connect plugins, manage MCP servers, and assign skills." className="hiveory-capability-header" actions={<HiveoryIconButton type="button" className="hiveory-capability-refresh" onClick={() => activeTab === 'skills' ? window.dispatchEvent(new Event('hiveory-refresh-skills')) : void refresh()} aria-label={`Refresh ${activeTab}`} disabled={busy !== null}><RefreshCw size={18} /></HiveoryIconButton>} />
    <HiveoryTabs label="Capability sections" value={activeTab} onChange={setActiveTab} tabs={[{ value: 'plugins', label: 'Plugins' }, { value: 'mcp', label: 'MCP' }, { value: 'skills', label: 'Skills' }]} />
    {activeTab === 'plugins' && <>
      <div className="hiveory-capability-toolbar">
        <HiveoryTabs label="Plugin views" value={pluginSubtab} onChange={selectPluginSubtab} tabs={[{ value: 'manual', label: 'Manual' }, { value: 'connected', label: `Connected ${connections.length}` }]} />
        <label className="hiveory-capability-search"><Search size={17} aria-hidden="true" /><HiveorySearchField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search plugins" aria-label="Search plugins" /></label>
      </div>
      <div className="hiveory-capability-section-heading"><h2>{pluginSubtab === 'manual' ? 'Available plugins' : 'Connected plugins'}</h2><div className="hiveory-capability-actions"><HiveoryButton type="button" intent="secondary" onClick={() => setShowCreator(true)} disabled={busy !== null}><Plus size={15} />Add custom</HiveoryButton><HiveoryButton type="button" intent="secondary" onClick={() => void importManifest()} disabled={busy !== null}>Import</HiveoryButton></div></div>
      <div className="hiveory-plugin-catalog-grid" aria-label={pluginSubtab === 'manual' ? 'Available plugins' : 'Connected plugins'}>
        {visibleCatalog.map((entry) => {
          ...
          return <article key={entry.manifest.id} className="hiveory-plugin-provider">
            <div className="hiveory-plugin-provider-main">{pluginMark(entry)}<button className="hiveory-plugin-provider-copy" onClick={() => openConfiguration(entry.manifest.id)}><strong>{entry.manifest.name}</strong><small>{entry.manifest.description}</small></button><div ref={openMenuId === entry.manifest.id ? menuRef : undefined} className="hiveory-plugin-menu-wrap"><HiveoryIconButton type="button" aria-label={`More options for ${entry.manifest.name}`} aria-haspopup="menu" aria-expanded={openMenuId === entry.manifest.id} onClick={() => setOpenMenuId((current) => current === entry.manifest.id ? null : entry.manifest.id)}><MoreHorizontal size={18} /></HiveoryIconButton>{openMenuId === entry.manifest.id && <HiveoryMenu className="hiveory-plugin-menu" label={`Options for ${entry.manifest.name}`} onClose={() => setOpenMenuId(null)}><HiveoryMenuItem onClick={() => openConfiguration(entry.manifest.id)}>Configure</HiveoryMenuItem><HiveoryMenuItem onClick={() => openConnection(entry.manifest.id)} disabled={!entry.enabled}>Add connection</HiveoryMenuItem><HiveoryMenuItem onClick={() => void action(`install-${entry.manifest.id}`, () => hiveoryClient.installPlugin({ plugin_id: entry.manifest.id, enabled: !entry.enabled }), entry.enabled ? `${entry.manifest.name} disabled.` : `${entry.manifest.name} enabled.`)}>{entry.enabled ? 'Disable' : 'Enable'}</HiveoryMenuItem>{entry.origin === 'user' && <HiveoryMenuItem intent="danger" onClick={() => void deletePlugin(entry)} disabled={busy !== null}>Delete</HiveoryMenuItem>}</HiveoryMenu>}</div><button className="hiveory-plugin-connect-button" onClick={() => openConnection(entry.manifest.id)} disabled={!entry.enabled || busy !== null}>{pluginConnections.length ? 'Add account' : 'Connect'}</button></div>
        ...
        {!visibleCatalog.length && <HiveoryEmptyState title={query ? 'No plugins match your search.' : pluginSubtab === 'connected' ? 'No connections yet.' : 'Plugins are loading.'}><>{!query && pluginSubtab === 'connected' && 'Connect a plugin to manage its accounts here.'}</></HiveoryEmptyState>}
      </div>
    </>}
    {activeTab === 'mcp' && <HiveoryEmptyState title="MCP server management is not available yet.">Hiveory currently supplies its MCP bridge through the active session.</HiveoryEmptyState>}
    ...
    {showCreator && <PluginCreator draft={creator} busy={busy === 'create'} onChange={setCreator} onCancel={() => { if (busy !== 'create') { setShowCreator(false); setCreator(emptyPluginDraft) } }} onCreate={() => void createPlugin()} />}
  </section>
}
...
function PluginCreator({ draft, busy, onChange, onCancel, onCreate }: { draft: PluginDraft; busy: boolean; onChange: (draft: PluginDraft) => void; onCancel: () => void; onCreate: () => void }) {
  ...
  return <HiveoryDialog title="Create plugin" description="Create a local, allow-listed HTTPS adapter. You can add a user-owned credential afterward." open onClose={onCancel} size="narrow" closeDisabled={busy} actions={<><HiveoryButton type="button" intent="secondary" onClick={onCancel} disabled={busy}>Cancel</HiveoryButton><HiveoryButton type="button" intent="primary" onClick={onCreate} disabled={busy || !draft.id.trim() || !draft.name.trim() || !draft.description.trim() || !draft.host.trim() || !draft.toolName.trim()}><Plus size={15} />{busy ? 'Creating…' : 'Create plugin'}</HiveoryButton></>}><div className="hiveory-form-stack"><label>Identifier<input value={draft.id} onChange={(event) => update('id', event.target.value)} placeholder="status-api" autoFocus maxLength={64} /><small>Lowercase letters, numbers, and dashes.</small></label><label>Name<input value={draft.name} onChange={(event) => update('name', event.target.value)} placeholder="Status API" maxLength={80} /></label><label>Description<input value={draft.description} onChange={(event) => update('description', event.target.value)} placeholder="What this plugin can do" maxLength={240} /></label><label>Allowed HTTPS host<input value={draft.host} onChange={(event) => update('host', event.target.value)} placeholder="api.example.com" maxLength={255} /></label><label>Tool name<input value={draft.toolName} onChange={(event) => update('toolName', event.target.value)} placeholder="get" maxLength={64} /></label><label>Method<select value={draft.method} onChange={(event) => update('method', event.target.value as PluginDraft['method'])}><option value="json_http_get">Read-only GET</option><option value="json_http_post">Approved POST with dry run</option></select></label></div></HiveoryDialog>
}
