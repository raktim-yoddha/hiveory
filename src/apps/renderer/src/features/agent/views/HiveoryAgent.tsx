...
export function HiveoryAgent() {
  ...
  useEffect(() => {
    const handleCopyResponseShortcut = (event: KeyboardEvent) => {
      ...
      if (target?.matches('input, textarea, [contenteditable="true"]') || target?.closest('.code-terminal-container, .hiveory-xterm, .hiveory-xterm-canvas')) return
