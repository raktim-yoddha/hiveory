...
import { hiveoryClient, type AgentFolderGrant, type AgentPluginGrant, type AgentSummary, type PluginCatalogEntry, type RoutineCreateRequest, type RoutineDetail, type RoutineExecution, type RoutineSummary, type RoutineUpdateRequest } from '../../../shared/api/hiveory-client'
...
function RoutineFormDialog({ agents, initial, busy, onCancel, onSave }: { agents: AgentSummary[]; initial: RoutineDetail | null; busy: boolean; onCancel: () => void; onSave: (request: RoutineCreateRequest | RoutineUpdateRequest) => void }) {
  ...
  useEffect(() => { void Promise.all([hiveoryClient.pluginCatalog(), form.agent_id ? hiveoryClient.agent(form.agent_id) : Promise.resolve(null), form.agent_id ? hiveoryClient.agentPluginGrants(form.agent_id) : Promise.resolve([])]).then(([catalog, agent, grants]) => { setPlugins(catalog.filter((plugin) => plugin.enabled)); setFolders(agent?.folders ?? []); setAgentGrants(grants.filter((grant) => grant.enabled)) }).catch(() => { setPlugins([]); setFolders([]); setAgentGrants([]) }) }, [form.agent_id])
