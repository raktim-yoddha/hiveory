import { Check, CircleCheck, Eye, KeyRound, Link2, LockKeyhole, MoreHorizontal, PlugZap, Plus, Puzzle, RefreshCw, Search, TestTube2, Trash2, UserRound, X } from 'lucide-react'
...
export function HiveoryPlugins() {
  ...
  return <section className="hiveory-plugin-page hiveory-plugin-page-list" aria-labelledby="hiveory-plugins-title">
    <header className="hiveory-plugin-page-toolbar">
      <div className="hiveory-plugin-page-title"><Puzzle size={17} aria-hidden="true" /><div><h1 id="hiveory-plugins-title">Plugins</h1><p>Local integrations available to Hiveory sessions.</p></div></div>
    ...
    <div className="hiveory-plugin-full-list" aria-label="Available local plugins">
      ...
      {!visibleCatalog.length && <div className="hiveory-empty-panel"><Puzzle size={24} />{catalog.length ? <p>No plugins match your search.</p> : <><p>Plugins are loading from the local runtime.</p><small>Refresh after the desktop runtime finishes starting.</small></>}</div>}
    </div>
