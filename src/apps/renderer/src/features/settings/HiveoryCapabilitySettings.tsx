...
type SettingsSection = 'onboarding' | 'agents' | 'orchestration' | 'computer-use' | 'general' | 'unavailable'
...
version: 1.0.0
description: Use Hiveory's local Browser and desktop workspaces to inspect and operate user-authorized local application surfaces.
triggers: [browser, desktop, computer]
permissions: [browser.navigate, workspace.read]
---

# Hiveory Computer Use

Use Hiveory Browser panes for web work and report the target, action, and result. Stay inside the workspace the user selected. Before changing external state, use the agent's configured approval policy and make the intended action explicit.`
...
export function HiveoryCapabilitySettings({
  ...
  const refresh = async () => {
    ...
    setError(null)
    try {
      const [nextSnapshot, nextPlugins, nextConnections, skills] = await Promise.all([
  ...
  const updatePreferences = (patch: Partial<CapabilityPreferences>) => setPreferences((current) => ({ ...current, ...patch }))
  ...
  const steps: Array<{ id: OnboardingStep; label: string; done: boolean }> = [
    ...
    { id: 'cli', label: 'Enable Hiveory CLI & skills', done: detectedAdapters.length > 0 && orchestrationInstalled && computerUseInstalled },
    ...
    { id: 'browser', label: 'Use Hiveory browser', done: true },
  ]
  ...
  const visibleContent = section === 'onboarding'
    ? <OnboardingPanel
        ...
        onInstallSkills={() => void Promise.all([installSkill('Hiveory Orchestration', orchestrationSkill), installSkill('Hiveory Computer Use', computerUseSkill)])}
  ...
  return <div className="hiveory-capability-settings">
    <aside className="hiveory-capability-sidebar" aria-label="Settings navigation">
      ...
      <div className="hiveory-capability-nav">
        {filteredSections.slice(0, 1).map((item) => <SettingsNavButton key={item.id} item={item} section={section} onSelect={setSection} />)}
        <p>AI capabilities</p>
        {filteredSections.filter((item) => ['agents', 'orchestration', 'computer-use'].includes(item.id)).map((item) => <SettingsNavButton key={item.id} item={item} section={section} onSelect={setSection} />)}
  ...
  return <section className="hiveory-capability-page hiveory-onboarding" aria-labelledby="hiveory-onboarding-title">
    ...
    <div className="hiveory-onboarding-layout">
      ...
      <article className="hiveory-onboarding-detail">
        ...
        {selectedStep === 'browser' && <><h2>Use Hiveory browser</h2><p>Browse local and web applications inside a workspace pane. Browser panes use the app’s in-app surface and start with Google.</p><div className="hiveory-browser-illustration"><Globe2 size={22} /><span /><span /><i /></div><button type="button" onClick={onOpenWorkbench}><Globe2 size={15} />Open Browser pane</button></>}
      </article>
    </div>
  </section>
}
