...
permissions: [browser.navigate, browser.interact, workspace.read, computer.run]
---
...
Use browser.open_external when the user explicitly asks for the current page in another browser. Use computer.run with target desktop for user-authorized external browser or desktop applications, for example opening a URL with the operating system. Never hide an external side effect: describe the target and action and follow the Agent approval policy before executing it.`

const browserUseSkill = `---
id: hiveory-browser-use
name: Hiveory Browser Use
version: 1.0.0
description: Drive the embedded Hiveory Browser or hand work to an external browser with explicit user authorization.
triggers: [browser, web, website]
permissions: [browser.navigate, browser.interact, browser.capture]
---

# Hiveory Browser Use

For pages inside Hiveory, use browser.state before acting and use the browser.* tools with the active browser_id. Use browser.open to reconnect a pane, browser.navigate for a URL, browser.click/fill/press for page actions, browser.scroll for movement, and browser.capture when the user needs a screenshot checkpoint. Use browser.open_external only when the user requests another browser. For desktop-wide actions, use computer.run with target desktop and the configured approval policy.`
...
function BrowserUsePanel({ installed, busy, preferences, onInstall, onRefresh, onOpenWorkbench, onOpenExternal, onUpdate }: { installed: boolean; busy: string | null; preferences: CapabilityPreferences; onInstall: () => void; onRefresh: () => void; onOpenWorkbench: () => void; onOpenExternal: (url: string) => void; onUpdate: (patch: Partial<CapabilityPreferences>) => void }) {
  ...
  return <section className="hiveory-capability-page" aria-labelledby="hiveory-browser-use-title"><header><h1 id="hiveory-browser-use-title">Browser Use</h1><p>Give agents a real browser surface and a controlled path to the rest of the user's computer.</p></header><section className="hiveory-capability-surface"><div className="hiveory-skill-hero"><span><Globe2 size={23} /></span><div><h2>Browser Use skill <b className={installed ? 'is-connected' : ''}>{installed ? 'Installed' : 'Not installed'}</b></h2><p>Agents can inspect and operate the embedded Hiveory Browser, open a page in an external browser, or use the approved desktop command tool.</p><div><button type="button" onClick={onInstall} disabled={busy !== null || installed}><Terminal size={15} />{installed ? 'Installed' : 'Install'}</button><button type="button" className="is-secondary" onClick={onRefresh} disabled={busy !== null}><RefreshCw size={15} />Re-check</button></div></div></div><SettingsRow title="Enable Agent Browser Use" detail="Expose browser.* tools to agents and include browser target guidance in new runs."><Toggle checked={preferences.browserUseEnabled} onChange={(checked) => onUpdate({ browserUseEnabled: checked })} label="Enable Agent Browser Use" /></SettingsRow><SettingsRow title="Browser target" detail="Choose where browser work should happen for the next agent run."><div className="hiveory-segmented hiveory-browser-target-segment"><button type="button" className={preferences.browserTarget === 'inner' ? 'is-selected' : ''} onClick={() => onUpdate({ browserTarget: 'inner' })}><Globe2 size={14} />Inner Browser</button><button type="button" className={preferences.browserTarget === 'external' ? 'is-selected' : ''} onClick={() => onUpdate({ browserTarget: 'external' })}><Globe2 size={14} />External browser</button><button type="button" className={preferences.browserTarget === 'desktop' ? 'is-selected' : ''} onClick={() => onUpdate({ browserTarget: 'desktop' })}><MonitorCog size={14} />User's PC</button></div></SettingsRow><div className="hiveory-browser-use-target-card"><strong>{preferences.browserTarget === 'inner' ? 'Embedded Hiveory Browser' : preferences.browserTarget === 'external' ? 'External browser handoff' : 'Desktop computer use'}</strong><p>{preferences.browserTarget === 'inner' ? 'Agents use browser.state, browser.navigate, browser.click, browser.fill, browser.press, browser.scroll, and browser.capture against the active Browser pane.' : preferences.browserTarget === 'external' ? 'Agents may open the current page in the system default browser. Existing browser sessions remain outside Hiveory and are controlled only through the approved desktop path.' : 'Agents use computer.run with target desktop for user-authorized browsers and other local apps. Mutating actions continue to follow the agent approval policy.'}</p>{preferences.browserTarget === 'inner' && <button type="button" onClick={onOpenWorkbench}><Globe2 size={15} />Open Browser pane</button>}{preferences.browserTarget === 'external' && <div className="hiveory-browser-external-form"><label>Page to open<input value={url} onChange={(event) => setUrl(event.target.value)} onBlur={saveUrl} placeholder="https://www.google.com/" /></label><button type="button" onClick={() => { saveUrl(); onOpenExternal(url) }} disabled={!url.trim()}><Globe2 size={15} />Open external browser</button></div>}{preferences.browserTarget === 'desktop' && <button type="button" className="is-secondary" onClick={() => onUpdate({ browserTarget: 'desktop' })}><MonitorCog size={15} />Desktop target selected</button>}</div><div className="hiveory-capability-note"><ShieldCheck size={17} /><span>Browser and desktop actions are visible in the Agent run timeline. Hiveory asks for approval according to the selected policy before external side effects.</span></div></section></section>
}
