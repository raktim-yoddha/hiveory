...
export function HiveoryWorkspaceBoard({ onOpenWorkspace, onStartLocalWork, onClose }: { onOpenWorkspace: (workspaceId: string) => void; onStartLocalWork: () => void; onClose: () => void }) {
  ...
  return <section className="hiveory-workspace-board" role="dialog" aria-modal="true" aria-label="Task board">
    <header className="hiveory-workspace-board-header"><h1>Workspace board</h1><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search local tasks" aria-label="Search local tasks" /><div className="hiveory-workspace-board-actions">{!loading && !tasks.length && <button type="button" className="hiveory-workspace-board-start" onClick={onStartLocalWork}>Open workspace</button>}<button type="button" title="Refresh board" aria-label="Refresh board" onClick={() => void refresh()}><RefreshCw size={16} className={loading ? 'is-spinning' : ''} /></button><button type="button" title="Close board" aria-label="Close board" onClick={onClose}><X size={17} /></button></div></header>
    ...
    {!loading && !tasks.length && <div className="hiveory-workspace-board-onboarding"><strong>No local tasks yet</strong><span>Tasks appear here when a Hiveory code run starts. Open a local workspace to begin one.</span><button type="button" onClick={onStartLocalWork}>Open workspace</button></div>}
