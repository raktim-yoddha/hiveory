...
export const HiveoryCodeSkills: React.FC = () => {
  ...
  return <section className="code-page-container hiveory-skills-page" aria-labelledby="hiveory-code-skills-title">
    <header className="code-page-header hiveory-skills-header">
      <div><h1 id="hiveory-code-skills-title" className="code-page-title">Skills</h1><p className="code-page-subtitle">Skills are installed once on this device and auto-match every local CLI. Turn one on here to always load it for a selected agent.</p></div>
...
function SkillCreator({ draft, busy, onChange, onCancel, onCreate }: { draft: SkillDraft; busy: boolean; onChange: (draft: SkillDraft) => void; onCancel: () => void; onCreate: () => void }) {
  ...
  return <div className="hiveory-modal-backdrop" role="presentation"><section className="hiveory-modal hiveory-skill-creator" role="dialog" aria-modal="true" aria-labelledby="hiveory-skill-creator-title"><div className="hiveory-modal-heading"><div><p className="hiveory-eyebrow">Local SKILL.md</p><h2 id="hiveory-skill-creator-title">Create a skill</h2></div><button className="hiveory-icon-button" onClick={onCancel} aria-label="Close skill creator"><X size={17} /></button></div><p>This creates a validated SKILL.md package in Hiveory’s local application data. It is then available to every CLI on this device.</p><div className="hiveory-form-grid"><label>Identifier<input value={draft.id} onChange={(event) => update('id', event.target.value)} placeholder="release-check" autoFocus maxLength={64} /><small>Lowercase letters, numbers, and dashes.</small></label><label>Name<input value={draft.name} onChange={(event) => update('name', event.target.value)} placeholder="Release check" maxLength={80} /></label><label className="is-wide">Description<input value={draft.description} onChange={(event) => update('description', event.target.value)} placeholder="What this skill does" maxLength={240} /></label><label className="is-wide">Trigger phrases<input value={draft.triggers} onChange={(event) => update('triggers', event.target.value)} placeholder="release readiness, check release" maxLength={240} /><small>Comma-separated phrases that automatically activate the skill.</small></label><label className="is-wide">Instructions<textarea value={draft.instructions} onChange={(event) => update('instructions', event.target.value)} rows={8} placeholder="Tell the local CLI exactly how to perform this task." maxLength={64 * 1024} /></label></div><div className="hiveory-modal-actions"><button className="is-secondary" onClick={onCancel}>Cancel</button><button disabled={busy || !draft.id.trim() || !draft.name.trim() || !draft.description.trim() || !draft.instructions.trim()} onClick={onCreate}><Check size={15} />{busy ? 'Creating…' : 'Create skill'}</button></div></section></div>
}
