...
impl HiveoryPersistence {
    pub async fn orchestration_runs(
        ...
        let rows = if let Some(workspace_id) = workspace_id {
            sqlx::query(
                "SELECT id, workspace_id, title, objective, model, state, review_policy, concurrency_limit, host_concurrency_cap, created_at_unix_ms, updated_at_unix_ms, error, coordinator_id, adapter_id FROM hiveory_code_runs WHERE workspace_id=? ORDER BY updated_at_unix_ms DESC",
            )
            .bind(workspace_id)
            .fetch_all(self.pool())
            .await?
            ...
            sqlx::query(
                "SELECT id, workspace_id, title, objective, model, state, review_policy, concurrency_limit, host_concurrency_cap, created_at_unix_ms, updated_at_unix_ms, error, coordinator_id, adapter_id FROM hiveory_code_runs ORDER BY updated_at_unix_ms DESC",
            )
            .fetch_all(self.pool())
            .await?
        };
    ...
    pub async fn orchestration_run(
        ...
        let Some(row) = sqlx::query(
            "SELECT id, workspace_id, title, objective, model, state, review_policy, concurrency_limit, host_concurrency_cap, created_at_unix_ms, updated_at_unix_ms, error, coordinator_id, adapter_id FROM hiveory_code_runs WHERE id=?",
        )
        .bind(run_id)
    ...
    pub async fn orchestration_detail(
        ...
        let tasks = sqlx::query(
            "SELECT id, run_id, client_id, title, specification, state, position, active_dispatch_id, latest_checkpoint_id, base_checkpoint_id, attempt, error, created_at_unix_ms, updated_at_unix_ms FROM hiveory_code_tasks WHERE run_id=? ORDER BY position, id",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let dependencies = sqlx::query(
            "SELECT run_id, task_id, depends_on_task_id FROM hiveory_code_task_dependencies WHERE run_id=? ORDER BY task_id, depends_on_task_id",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let dispatches = sqlx::query(
            "SELECT id, run_id, task_id, attempt, state, lease_generation, session_id, pid, worktree_id, checkpoint_id, last_heartbeat_at_unix_ms, started_at_unix_ms, updated_at_unix_ms, error, result_summary, adapter_id, terminal_id, cancel_requested_at_unix_ms FROM hiveory_code_dispatches WHERE run_id=? ORDER BY updated_at_unix_ms DESC",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let worktrees = sqlx::query(
            "SELECT id, run_id, task_id, dispatch_id, path, branch, base_checkpoint_id, state, dirty, locked, error, created_at_unix_ms, updated_at_unix_ms FROM hiveory_code_worktrees WHERE run_id=? ORDER BY created_at_unix_ms DESC",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let checkpoints = sqlx::query(
            "SELECT id, run_id, task_id, dispatch_id, kind, state, ref_name, commit_oid, parent_checkpoint_id, summary, created_at_unix_ms FROM hiveory_code_checkpoints WHERE run_id=? ORDER BY created_at_unix_ms DESC",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let reviews = sqlx::query(
            "SELECT id, run_id, task_id, checkpoint_id, decision, feedback, created_at_unix_ms FROM hiveory_code_reviews WHERE run_id=? ORDER BY created_at_unix_ms DESC",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let questions = sqlx::query(
            "SELECT id, run_id, task_id, dispatch_id, prompt, answer, answered, created_at_unix_ms FROM hiveory_code_questions WHERE run_id=? ORDER BY created_at_unix_ms DESC",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let messages = sqlx::query(
            "SELECT id, run_id, task_id, dispatch_id, kind, question_id, payload, created_at_unix_ms FROM hiveory_code_messages WHERE run_id=? ORDER BY created_at_unix_ms DESC LIMIT 250",
        )
        .bind(run_id)
        .fetch_all(self.pool())
        .await?
        ...
        let proposal = sqlx::query("SELECT proposal_json FROM hiveory_code_runs WHERE id=?")
            .bind(run_id)
            .fetch_one(self.pool())
            .await?
    ...
    pub async fn insert_orchestration_run(
        ...
        let now = now_ms();
        sqlx::query(
            "INSERT INTO hiveory_code_runs (id, workspace_id, title, objective, state, review_policy, model, concurrency_limit, host_concurrency_cap, created_at_unix_ms, updated_at_unix_ms, coordinator_id, adapter_id, next_event_sequence) VALUES (?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?, 1)",
        )
        .bind(run_id)
    ...
    pub async fn update_orchestration_run(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_runs SET title=?, objective=?, review_policy=?, concurrency_limit=?, updated_at_unix_ms=? WHERE id=? AND state IN ('draft','ready','blocked','interrupted')",
        )
        .bind(title.trim())
        .bind(objective.trim())
    ...
    pub async fn save_orchestration_proposal(
        ...
        sqlx::query(
            "UPDATE hiveory_code_runs SET proposal_json=?, updated_at_unix_ms=? WHERE id=?",
        )
        .bind(json)
        .bind(now_ms())
        .bind(run_id)
    ...
    pub async fn set_orchestration_source_checkpoint(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_runs SET source_checkpoint_id=?, updated_at_unix_ms=? WHERE id=?",
        )
        .bind(checkpoint_id)
        .bind(now_ms())
        .bind(run_id)
    ...
    pub async fn set_orchestration_task_state(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_tasks SET state=?, active_dispatch_id=?, base_checkpoint_id=COALESCE(?, base_checkpoint_id), latest_checkpoint_id=COALESCE(?, latest_checkpoint_id), attempt=COALESCE(?, attempt), error=?, updated_at_unix_ms=? WHERE run_id=? AND id=?",
        )
    ...
    pub async fn set_orchestration_run_state(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_runs SET state=?, error=?, updated_at_unix_ms=? WHERE id=?",
        )
    ...
    pub async fn cancel_active_orchestration(
        ...
        let now = now_ms();
        sqlx::query(
            "UPDATE hiveory_code_dispatches SET state='cancelled', error=?, cancel_requested_at_unix_ms=?, updated_at_unix_ms=? WHERE run_id=? AND state IN ('preparing','running','awaiting_input','checkpointing')",
        )
        .bind(reason)
        .bind(now)
        .bind(now)
        ...
        .await?;
        sqlx::query(
            "UPDATE hiveory_code_tasks SET state='cancelled', active_dispatch_id=NULL, error=?, updated_at_unix_ms=? WHERE run_id=? AND state IN ('preparing','running','awaiting_input')",
        )
        .bind(reason)
        .bind(now)
        .bind(run_id)
    ...
    pub async fn cancel_orchestration_dispatch(
        ...
        let now = now_ms();
        let result = sqlx::query(
            "UPDATE hiveory_code_dispatches SET state='cancelled', error=?, cancel_requested_at_unix_ms=?, updated_at_unix_ms=? WHERE id=? AND run_id=? AND task_id=? AND lease_generation=? AND state IN ('preparing','running','awaiting_input','checkpointing')",
        )
        .bind(reason)
        .bind(now)
        .bind(now)
        ...
        if result.rows_affected() == 1 {
            sqlx::query(
                "UPDATE hiveory_code_tasks SET state='cancelled', active_dispatch_id=NULL, error=?, updated_at_unix_ms=? WHERE id=? AND run_id=? AND active_dispatch_id=?",
            )
            .bind(reason)
            .bind(now)
            .bind(task_id)
    ...
    pub async fn attach_orchestration_terminal(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_dispatches SET terminal_id=?, updated_at_unix_ms=? WHERE id=? AND lease_generation=? AND state NOT IN ('succeeded','failed','cancelled','stale')",
        )
        .bind(terminal_id)
        .bind(now_ms())
        .bind(dispatch_id)
    ...
    pub async fn mark_stale_orchestration_dispatches(
        ...
        let mut tx = self.pool().begin().await?;
        let rows = sqlx::query(
            "SELECT id, run_id, task_id FROM hiveory_code_dispatches WHERE run_id=? AND state IN ('preparing','running','checkpointing') AND last_heartbeat_at_unix_ms IS NOT NULL AND last_heartbeat_at_unix_ms < ?",
        )
        .bind(run_id)
        ...
        for row in rows {
            ...
            sqlx::query(
                "UPDATE hiveory_code_dispatches SET state='stale', error='Worker heartbeat expired', updated_at_unix_ms=? WHERE id=? AND state IN ('preparing','running','checkpointing')",
            )
            .bind(now)
            .bind(&dispatch_id)
            .execute(&mut *tx)
            .await?;
            sqlx::query(
                "UPDATE hiveory_code_tasks SET state='failed', active_dispatch_id=NULL, error='Worker heartbeat expired', updated_at_unix_ms=? WHERE id=? AND run_id=? AND active_dispatch_id=?",
            )
            .bind(now)
            .bind(&task_id)
            .bind(&run_id)
            ...
            .await?;
            sqlx::query(
                "UPDATE hiveory_code_runs SET state='failed', error='A worker heartbeat expired', updated_at_unix_ms=? WHERE id=? AND state='running'",
            )
            .bind(now)
            .bind(&run_id)
            .execute(&mut *tx)
    ...
    pub async fn insert_orchestration_task(
        ...
        let mut tx = self.pool().begin().await?;
        sqlx::query(
            "INSERT INTO hiveory_code_tasks (id, run_id, client_id, title, specification, state, position, active_dispatch_id, latest_checkpoint_id, base_checkpoint_id, attempt, error, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&task.id)
        .bind(&task.run_id)
        .bind(&task.client_id)
        ...
        for dependency in dependencies {
            sqlx::query(
                "INSERT INTO hiveory_code_task_dependencies (run_id, task_id, depends_on_task_id) VALUES (?, ?, ?)",
            )
    ...
    pub async fn update_orchestration_task(
        ...
        let mut tx = self.pool().begin().await?;
        let result = sqlx::query(
            "UPDATE hiveory_code_tasks SET client_id=?, title=?, specification=?, state=?, position=?, active_dispatch_id=?, latest_checkpoint_id=?, base_checkpoint_id=?, attempt=?, error=?, updated_at_unix_ms=? WHERE id=? AND run_id=? AND state IN ('draft','ready','blocked','failed','interrupted')",
        )
        .bind(&task.client_id)
        .bind(&task.title)
        .bind(&task.specification)
        ...
        .await?;
        if result.rows_affected() == 1 {
            sqlx::query("DELETE FROM hiveory_code_task_dependencies WHERE run_id=? AND task_id=?")
                .bind(&task.run_id)
                .bind(&task.id)
                .execute(&mut *tx)
                .await?;
    ...
    pub async fn delete_orchestration_task(
        ...
        let result =
            sqlx::query("DELETE FROM hiveory_code_tasks WHERE run_id=? AND id=? AND state='draft'")
                .bind(run_id)
                .bind(task_id)
                .execute(self.pool())
                .await?;
    ...
    pub async fn claim_orchestration_dispatch(
        ...
        let mut tx = self.pool().begin().await?;
        let result = sqlx::query(
            "UPDATE hiveory_code_tasks SET state='preparing', active_dispatch_id=?, attempt=?, error=NULL, updated_at_unix_ms=? WHERE id=? AND run_id=? AND state='ready' AND active_dispatch_id IS NULL",
        )
        .bind(&dispatch.id)
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_dispatches (id, run_id, task_id, attempt, state, adapter_id, lease_generation, session_id, pid, worktree_id, checkpoint_id, last_heartbeat_at_unix_ms, terminal_id, cancel_requested_at_unix_ms, started_at_unix_ms, updated_at_unix_ms, error, result_summary) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&dispatch.id)
        .bind(&dispatch.run_id)
        .bind(&dispatch.task_id)
    ...
    pub async fn set_orchestration_task_result(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_tasks SET state=?, latest_checkpoint_id=COALESCE(?, latest_checkpoint_id), error=?, updated_at_unix_ms=? WHERE run_id=? AND id=? AND active_dispatch_id=? AND state IN ('preparing','running','awaiting_input','awaiting_review','checkpointing') AND EXISTS (SELECT 1 FROM hiveory_code_dispatches WHERE id=? AND run_id=? AND task_id=? AND lease_generation=? AND state IN ('preparing','running','awaiting_input','checkpointing','succeeded'))",
        )
        .bind(task_state_value(state))
        .bind(checkpoint_id)
        .bind(error)
    ...
    pub async fn resume_orchestration_dispatch(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_dispatches SET state='running', lease_generation=lease_generation+1, updated_at_unix_ms=?, error=NULL WHERE id=? AND run_id=? AND task_id=? AND state='awaiting_input' AND lease_generation=?",
        )
        .bind(now_ms())
        .bind(dispatch_id)
        .bind(run_id)
        ...
        sqlx::query(
            "UPDATE hiveory_code_tasks SET state='running', error=NULL, updated_at_unix_ms=? WHERE run_id=? AND id=? AND active_dispatch_id=?",
        )
        .bind(now_ms())
        .bind(run_id)
        .bind(task_id)
        ...
        .await?;
        sqlx::query(
            "UPDATE hiveory_code_questions SET answer=?, answered=1 WHERE id=(SELECT id FROM hiveory_code_questions WHERE dispatch_id=? AND answered=0 ORDER BY created_at_unix_ms DESC LIMIT 1)",
        )
        .bind(answer)
        .bind(dispatch_id)
        .execute(&mut *tx)
    ...
    pub async fn resume_interrupted_orchestration_dispatch(
        ...
        let mut tx = self.pool().begin().await?;
        let result = sqlx::query(
            "UPDATE hiveory_code_dispatches SET state='running', lease_generation=lease_generation+1, updated_at_unix_ms=?, error=NULL WHERE id=? AND run_id=? AND task_id=? AND state='interrupted' AND lease_generation=?",
        )
        .bind(now_ms())
        .bind(dispatch_id)
        .bind(run_id)
    ...
    pub async fn insert_orchestration_worktree(
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_worktrees (id, run_id, task_id, dispatch_id, path, branch, base_checkpoint_id, state, dirty, locked, error, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&worktree.id)
    ...
    pub async fn update_orchestration_worktree(
        ...
        let result = sqlx::query(
            "UPDATE hiveory_code_worktrees SET state=?, dirty=?, locked=?, error=?, updated_at_unix_ms=? WHERE id=?",
        )
    ...
    pub async fn insert_orchestration_checkpoint(
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_checkpoints (id, run_id, task_id, dispatch_id, kind, state, ref_name, commit_oid, parent_checkpoint_id, summary, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&checkpoint.id)
    ...
    pub async fn insert_orchestration_review(
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_reviews (id, run_id, task_id, checkpoint_id, decision, feedback, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&review.id)
        .bind(&review.run_id)
        .bind(&review.task_id)
    ...
    pub async fn insert_orchestration_question(
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_questions (id, run_id, task_id, dispatch_id, prompt, answer, answered, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&question.id)
    ...
    pub async fn insert_orchestration_message(
        ...
        sqlx::query(
            "INSERT INTO hiveory_code_messages (id, run_id, task_id, dispatch_id, kind, question_id, payload, created_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&message.id)
        .bind(&message.run_id)
    ...
    pub async fn insert_orchestration_event(
        ...
        if let Some(nonce) = nonce {
            if let Some(row) = sqlx::query(
                "SELECT run_id, sequence, event_id, task_id, dispatch_id, lease_generation, kind, payload, accepted, origin, worker_sequence, nonce, emitted_at_unix_ms FROM hiveory_code_events WHERE nonce=?",
            )
            .bind(nonce)
        ...
        let allocated = sqlx::query(
            "UPDATE hiveory_code_runs SET next_event_sequence=next_event_sequence+1 WHERE id=?",
        )
        .bind(run_id)
        .execute(&mut *tx)
        .await?;
        ...
        let sequence: i64 =
            sqlx::query("SELECT next_event_sequence - 1 FROM hiveory_code_runs WHERE id=?")
                .bind(run_id)
                .fetch_one(&mut *tx)
                .await?
                .get(0);
        ...
        sqlx::query(
            "INSERT OR IGNORE INTO hiveory_code_events (run_id, sequence, event_id, task_id, dispatch_id, lease_generation, kind, payload, accepted, origin, worker_sequence, nonce, emitted_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(run_id)
        .bind(sequence)
        .bind(event_id)
        ...
        let row = if let Some(nonce) = nonce {
            sqlx::query(
                "SELECT run_id, sequence, event_id, task_id, dispatch_id, lease_generation, kind, payload, accepted, origin, worker_sequence, nonce, emitted_at_unix_ms FROM hiveory_code_events WHERE event_id=? OR nonce=? ORDER BY CASE WHEN event_id=? THEN 0 ELSE 1 END LIMIT 1",
            )
            .bind(event_id)
            .bind(nonce)
            .bind(event_id)
            ...
            sqlx::query(
                "SELECT run_id, sequence, event_id, task_id, dispatch_id, lease_generation, kind, payload, accepted, origin, worker_sequence, nonce, emitted_at_unix_ms FROM hiveory_code_events WHERE event_id=?",
            )
            .bind(event_id)
            .fetch_one(self.pool())
            .await?
    ...
    pub async fn orchestration_events(
        ...
        Ok(sqlx::query(
            "SELECT run_id, sequence, event_id, task_id, dispatch_id, lease_generation, kind, payload, accepted, origin, worker_sequence, nonce, emitted_at_unix_ms FROM hiveory_code_events WHERE run_id=? AND sequence>? ORDER BY sequence LIMIT ?",
        )
        .bind(run_id)
    ...
    pub async fn interrupt_active_orchestration(&self) -> Result<usize, sqlx::Error> {
        let now = now_ms();
        let runs = sqlx::query(
            "UPDATE hiveory_code_runs SET state='interrupted', updated_at_unix_ms=? WHERE state IN ('running','preparing')",
        )
        .bind(now)
        .execute(self.pool())
        .await?
        ...
        sqlx::query(
            "UPDATE hiveory_code_dispatches SET state='interrupted', updated_at_unix_ms=?, error='Application restarted before this dispatch finished' WHERE state IN ('preparing','running','checkpointing')",
        )
        .bind(now)
        .execute(self.pool())
        .await?;
        sqlx::query(
            "UPDATE hiveory_code_tasks SET state='failed', error='Application restarted before this task finished', updated_at_unix_ms=? WHERE state IN ('preparing','running')",
        )
        .bind(now)
        .execute(self.pool())
        .await?;
    ...
    async fn orchestration_summary_from_row(
        ...
        let task_count: i64 = sqlx::query("SELECT COUNT(*) FROM hiveory_code_tasks WHERE run_id=?")
            .bind(&id)
            .fetch_one(self.pool())
            .await?
            .get(0);
        let completed_tasks: i64 = sqlx::query(
            "SELECT COUNT(*) FROM hiveory_code_tasks WHERE run_id=? AND state='completed'",
        )
        .bind(&id)
        .fetch_one(self.pool())
        .await?
        ...
        let active_dispatches: i64 = sqlx::query(
            "SELECT COUNT(*) FROM hiveory_code_dispatches WHERE run_id=? AND state IN ('preparing','running','awaiting_input','checkpointing')",
        )
        .bind(&id)
        .fetch_one(self.pool())
        .await?
