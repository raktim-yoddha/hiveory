...
#[derive(Debug, Error)]
pub enum HiveoryRoutineStoreError {
...
#[derive(Clone)]
pub struct HiveoryRoutineStore {
    persistence: super::HiveoryPersistence,
}

impl HiveoryRoutineStore {
    ...
    pub async fn create(
    ...
    ) -> Result<RoutineDetail, HiveoryRoutineStoreError> {
        validate_request(
            &request.name,
        ...
        let result = sqlx::query(
            "INSERT INTO hiveory_routines (id, name, description, agent_id, prompt_template, schedule_expression, timezone, enabled, archived, catch_up, concurrency, delivery, folder_grant_ids_json, plugin_tool_names_json, max_duration_seconds, max_tool_calls, approval_timeout_seconds, next_run_unix_ms, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&id)
        ...
        match result {
            ...
            Err(sqlx::Error::Database(error)) if error.message().contains("FOREIGN KEY") => {
                Err(HiveoryRoutineStoreError::NotFound)
            }
            Err(sqlx::Error::Database(error)) if error.message().contains("UNIQUE") => {
                Err(HiveoryRoutineStoreError::Conflict)
            }
    ...
    pub async fn update(
        ...
        self.ensure_folder_grants(&request.agent_id, &request.folder_grant_ids)
            .await?;
        let result = sqlx::query(
            "UPDATE hiveory_routines SET name=?, description=?, agent_id=?, prompt_template=?, schedule_expression=?, timezone=?, enabled=?, catch_up=?, concurrency=?, delivery=?, folder_grant_ids_json=?, plugin_tool_names_json=?, max_duration_seconds=?, max_tool_calls=?, approval_timeout_seconds=?, next_run_unix_ms=?, updated_at_unix_ms=? WHERE id=? AND archived=0",
        )
        .bind(request.name.trim())
        .bind(request.description.trim())
        .bind(&request.agent_id)
        ...
        if result.rows_affected() == 0 {
            return Err(HiveoryRoutineStoreError::NotFound);
        }
    ...
    pub async fn set_archived(
        ...
        routine_id: &str,
        archived: bool,
    ) -> Result<(), HiveoryRoutineStoreError> {
        let result = sqlx::query(
            "UPDATE hiveory_routines SET archived=?, enabled=CASE WHEN ?=1 THEN 0 ELSE enabled END, updated_at_unix_ms=? WHERE id=?",
        )
    ...
    pub async fn execution_for_run(
        &self,
        run_id: &str,
    ) -> Result<Option<RoutineExecution>, HiveoryRoutineStoreError> {
        let row = sqlx::query("SELECT id, routine_id, run_id, occurrence_key, scheduled_for_unix_ms, state, folder_grant_ids_json, plugin_tool_names_json, error, report, created_at_unix_ms, updated_at_unix_ms, started_at_unix_ms, completed_at_unix_ms FROM hiveory_routine_executions WHERE run_id=? OR id=(SELECT routine_execution_id FROM hiveory_agent_runs WHERE id=?)")
            .bind(run_id)
            .bind(run_id)
    ...
    pub async fn execution_by_id(
        ...
        let row = sqlx::query("SELECT id, routine_id, run_id, occurrence_key, scheduled_for_unix_ms, state, folder_grant_ids_json, plugin_tool_names_json, error, report, created_at_unix_ms, updated_at_unix_ms, started_at_unix_ms, completed_at_unix_ms FROM hiveory_routine_executions WHERE id=?")
            .bind(execution_id)
            .fetch_optional(self.persistence.pool())
            .await?;
        row.map(execution_from_row).transpose()
    ...
    pub async fn advance_next_run(
        ...
        next_run_unix_ms: Option<i64>,
    ) -> Result<bool, HiveoryRoutineStoreError> {
        let result = sqlx::query("UPDATE hiveory_routines SET next_run_unix_ms=?, updated_at_unix_ms=? WHERE id=? AND enabled=1 AND archived=0 AND next_run_unix_ms=?")
            .bind(next_run_unix_ms)
            .bind(now_ms())
            .bind(routine_id)
    ...
    pub async fn claim_occurrence(
        ...
        let Some(row) = sqlx::query("SELECT concurrency, folder_grant_ids_json, plugin_tool_names_json FROM hiveory_routines WHERE id=? AND enabled=1 AND archived=0 AND next_run_unix_ms=?")
            .bind(routine_id)
            .bind(expected_next_run_unix_ms)
        ...
        let active: i64 = sqlx::query("SELECT COUNT(*) FROM hiveory_routine_executions WHERE routine_id=? AND state IN ('queued','running','awaiting_approval')")
            .bind(routine_id)
            .fetch_one(&mut *transaction)
            .await?
            .get(0);
        let queued: i64 = sqlx::query(
            "SELECT COUNT(*) FROM hiveory_routine_executions WHERE routine_id=? AND state='queued'",
        )
        .bind(routine_id)
        .fetch_one(&mut *transaction)
        .await?
        ...
        let result = sqlx::query("INSERT OR IGNORE INTO hiveory_routine_executions (id, routine_id, occurrence_key, scheduled_for_unix_ms, state, folder_grant_ids_json, plugin_tool_names_json, created_at_unix_ms, updated_at_unix_ms, completed_at_unix_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CASE WHEN ?='skipped' THEN ? ELSE NULL END)")
            .bind(&id)
            .bind(routine_id)
            .bind(occurrence_key)
        ...
        let advance = sqlx::query("UPDATE hiveory_routines SET next_run_unix_ms=?, last_run_unix_ms=?, last_execution_state=?, updated_at_unix_ms=? WHERE id=? AND enabled=1 AND archived=0 AND next_run_unix_ms=?")
            .bind(next_run_unix_ms)
            .bind(scheduled_for_unix_ms)
            .bind(state_value)
            .bind(now)
    ...
    pub async fn create_manual_execution(
        ...
        routine_id: &str,
        scheduled_for_unix_ms: i64,
    ) -> Result<RoutineExecution, HiveoryRoutineStoreError> {
        let row = sqlx::query("SELECT r.id, r.name, r.description, r.agent_id, COALESCE(a.name, 'Unknown Agent'), r.schedule_expression, r.timezone, r.enabled, r.archived, r.catch_up, r.concurrency, r.delivery, r.next_run_unix_ms, r.last_run_unix_ms, r.last_execution_state, r.created_at_unix_ms, r.updated_at_unix_ms, r.prompt_template, r.folder_grant_ids_json, r.plugin_tool_names_json, r.max_duration_seconds, r.max_tool_calls, r.approval_timeout_seconds FROM hiveory_routines r LEFT JOIN hiveory_agents a ON a.id=r.agent_id WHERE r.id=? AND r.archived=0")
            .bind(routine_id)
            .fetch_optional(self.persistence.pool())
            .await?
            .ok_or(HiveoryRoutineStoreError::NotFound)?;
        ...
        let id = Uuid::now_v7().to_string();
        let now = now_ms();
        sqlx::query("INSERT INTO hiveory_routine_executions (id, routine_id, occurrence_key, scheduled_for_unix_ms, state, folder_grant_ids_json, plugin_tool_names_json, created_at_unix_ms, updated_at_unix_ms) VALUES (?, ?, ?, ?, 'queued', ?, ?, ?, ?)")
            .bind(&id)
            .bind(routine_id)
    ...
    pub async fn link_execution_run(
        ...
        let result = sqlx::query("UPDATE hiveory_routine_executions SET run_id=?, state='running', started_at_unix_ms=COALESCE(started_at_unix_ms, ?), updated_at_unix_ms=? WHERE id=? AND state='queued'")
            .bind(run_id)
            .bind(now_ms())
            .bind(now_ms())
            .bind(execution_id)
            ...
            .await?;
        if result.rows_affected() == 0 {
            return Err(HiveoryRoutineStoreError::Conflict);
        }
    ...
    pub async fn set_execution_state(
        ...
        let now = now_ms();
        let result = sqlx::query("UPDATE hiveory_routine_executions SET state=?, error=?, report=COALESCE(?, report), updated_at_unix_ms=?, completed_at_unix_ms=CASE WHEN ?=1 THEN COALESCE(completed_at_unix_ms, ?) ELSE completed_at_unix_ms END WHERE id=?")
    ...
    pub async fn active_executions(
        ...
        let rows = sqlx::query("SELECT id, routine_id, run_id, occurrence_key, scheduled_for_unix_ms, state, folder_grant_ids_json, plugin_tool_names_json, error, report, created_at_unix_ms, updated_at_unix_ms, started_at_unix_ms, completed_at_unix_ms FROM hiveory_routine_executions WHERE state IN ('queued','running','awaiting_approval') ORDER BY created_at_unix_ms")
            .fetch_all(self.persistence.pool())
            .await?;
        rows.into_iter().map(execution_from_row).collect()
    }
    ...
    async fn routine_row(
        &self,
        routine_id: &str,
    ) -> Result<Option<SqliteRow>, HiveoryRoutineStoreError> {
        Ok(sqlx::query("SELECT r.id, r.name, r.description, r.agent_id, COALESCE(a.name, 'Unknown Agent'), r.schedule_expression, r.timezone, r.enabled, r.archived, r.catch_up, r.concurrency, r.delivery, r.next_run_unix_ms, r.last_run_unix_ms, r.last_execution_state, r.created_at_unix_ms, r.updated_at_unix_ms, r.prompt_template, r.folder_grant_ids_json, r.plugin_tool_names_json, r.max_duration_seconds, r.max_tool_calls, r.approval_timeout_seconds FROM hiveory_routines r LEFT JOIN hiveory_agents a ON a.id=r.agent_id WHERE r.id=?")
            .bind(routine_id)
            .fetch_optional(self.persistence.pool())
            .await?)
    }

    async fn ensure_agent(&self, agent_id: &str) -> Result<(), HiveoryRoutineStoreError> {
    ...
    async fn ensure_folder_grants(
        ...
        for grant_id in grant_ids {
            let exists = sqlx::query(
                "SELECT 1 FROM hiveory_agent_folders WHERE id=? AND agent_id=? AND (can_read=1 OR can_write=1)",
            )
            .bind(grant_id)
            .bind(agent_id)
            .fetch_optional(self.persistence.pool())
            ...
            .is_some();
            if !exists {
                return Err(HiveoryRoutineStoreError::InvalidInput(
...
fn execution_from_row(row: SqliteRow) -> Result<RoutineExecution, HiveoryRoutineStoreError> {
    Ok(RoutineExecution {
        id: row.get(0),
