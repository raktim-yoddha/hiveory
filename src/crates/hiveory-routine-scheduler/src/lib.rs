...
#[derive(Debug, Error)]
pub enum HiveoryRoutineSchedulerError {
    #[error("routine store failure: {0}")]
    Store(#[from] HiveoryRoutineStoreError),
...
#[async_trait]
impl RoutineRunLauncher for HiveoryAgentRuntime {
    async fn launch(&self, request: AgentRunStartRequest) -> Result<AgentRunSummary, String> {
...
#[derive(Clone)]
pub struct HiveoryRoutineScheduler {
    store: HiveoryRoutineStore,
    notifications: HiveoryNotificationService,
    launcher: Arc<dyn RoutineRunLauncher>,
    shutdown: CancellationToken,
}

impl HiveoryRoutineScheduler {
    pub fn new(
        ...
        Self {
            store: HiveoryRoutineStore::new(persistence),
            notifications,
            launcher,
    ...
    pub async fn list(
        &self,
        query: &RoutineQuery,
    ) -> Result<Vec<RoutineSummary>, HiveoryRoutineSchedulerError> {
    ...
    pub async fn detail(
        &self,
        routine_id: &str,
    ) -> Result<RoutineDetail, HiveoryRoutineSchedulerError> {
    ...
    ) -> Result<(), HiveoryRoutineSchedulerError> {
    ...
    pub async fn run_now(
        &self,
        routine_id: &str,
    ) -> Result<RoutineExecution, HiveoryRoutineSchedulerError> {
        let execution = self
            .store
    ...
    pub async fn executions(
        ...
        routine_id: &str,
        limit: u32,
    ) -> Result<Vec<RoutineExecution>, HiveoryRoutineSchedulerError> {
    ...
    async fn reconcile_active(&self) -> Result<(), HiveoryRoutineSchedulerError> {
            ...
            .filter(|execution| execution.run_id.is_some())
        {
            ...
            let Some(state) = self
                ...
                .state(run_id)
                .await
                .map_err(HiveoryRoutineSchedulerError::Launcher)?
            else {
                continue;
            };
            let delivery = self
    ...
    async fn dispatch_queued(&self) -> Result<(), HiveoryRoutineSchedulerError> {
        let active_executions = self.store.active_executions().await?;
        for execution in active_executions
            .iter()
            .filter(|execution| execution.run_id.is_none())
        {
            ...
            if detail.summary.concurrency == hiveory_protocol::RoutineConcurrencyPolicy::QueueOne
                && has_active_run
            {
                continue;
            }
    ...
    async fn dispatch(
        ...
        match self.launcher.launch(request).await {
            ...
            Err(error) => {
                ...
                .await?;
                Err(HiveoryRoutineSchedulerError::Launcher(error))
            }
        }
    }
    ...
    async fn notify(
        ...
        result
            .map(|_| ())
            .map_err(|error| HiveoryRoutineSchedulerError::Launcher(error.to_string()))
    }
}
...
pub fn next_occurrence(
    ...
    timezone: &str,
    after_unix_ms: i64,
) -> Result<Option<RoutineOccurrence>, HiveoryRoutineSchedulerError> {
    let cron = expression
        .parse::<CronExpression>()
        .map_err(HiveoryRoutineSchedulerError::InvalidSchedule)?;
    let tz = timezone.parse::<Tz>().map_err(|_| {
        HiveoryRoutineSchedulerError::InvalidSchedule(format!("unknown IANA timezone: {timezone}"))
    })?;
...
fn due_occurrences(
    ...
    first_unix_ms: i64,
    now_unix_ms: i64,
) -> Result<Vec<RoutineOccurrence>, HiveoryRoutineSchedulerError> {
