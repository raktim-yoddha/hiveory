...
pub fn preset_max_panes(preset: CodePanePreset) -> usize {
    match preset {
        ...
        CodePanePreset::Grid | CodePanePreset::Equal => 16,
...
pub fn apply_layout_preset_with_primary(
    ...
    fn build_balanced_stack(
        ...
        *split_counter += 1;
        ...
        let left_id =
            build_balanced_stack(left_leaves, orientation, split_counter, generated_nodes);
        let right_id =
            build_balanced_stack(right_leaves, orientation, split_counter, generated_nodes);
    ...
    fn build_row_group(
        row_roots: &[(String, usize)],
        split_counter: &mut usize,
        generated_nodes: &mut Vec<CodePaneNode>,
    ) -> String {
        if row_roots.is_empty() {
            panic!("build_row_group called with empty rows");
        }
        if row_roots.len() == 1 {
            return row_roots[0].0.clone();
        }

        let total_leaves: usize = row_roots.iter().map(|(_, count)| *count).sum();
        *split_counter += 1;
        let split_id = format!("split_{}", split_counter);

        let mid = row_roots.len() / 2;
        let top_rows = &row_roots[..mid];
        let bot_rows = &row_roots[mid..];

        let top_id = build_row_group(top_rows, split_counter, generated_nodes);
        let bot_id = build_row_group(bot_rows, split_counter, generated_nodes);

        let top_leaves: usize = top_rows.iter().map(|(_, count)| *count).sum();
        let top_ratio = ((top_leaves as f64 / total_leaves as f64) * 100.0).round() as u8;
        let ratio_percent = top_ratio.clamp(10, 90);

        for node in generated_nodes.iter_mut() {
            if node.pane_id == top_id || node.pane_id == bot_id {
                node.parent_id = Some(split_id.clone());
            }
        }
        ...
        generated_nodes.push(CodePaneNode {
            ...
            parent_id: None,
            kind: CodePaneKind::Empty,
            orientation: Some(CodePaneOrientation::Vertical),
            ratio_percent: Some(ratio_percent),
            children: vec![top_id, bot_id],
            resource_id: None,
            title: None,
        });
    ...
    fn build_equal_grid(
        ...
        let base_per_row = n / num_rows;
        let remainder = n % num_rows;

        let mut row_slices: Vec<&[CodePaneNode]> = Vec::new();
        let mut start = 0;
        for i in 0..num_rows {
            let count = base_per_row + if i < remainder { 1 } else { 0 };
            let end = start + count;
            row_slices.push(&leaves[start..end]);
            start = end;
        }

        let mut row_roots = Vec::new();
        for row_leaves in row_slices {
            let root_id = build_balanced_stack(
                row_leaves,
                CodePaneOrientation::Horizontal,
                split_counter,
                generated_nodes,
            );
            row_roots.push((root_id, row_leaves.len()));
        }

        build_row_group(&row_roots, split_counter, generated_nodes)
    }
    ...
    fn build_focus_layout(
        ...
        let right_root_id = if sup_count <= 4 {
            ...
            let col1_id = build_balanced_stack(
                col1_leaves,
                CodePaneOrientation::Vertical,
                split_counter,
                generated_nodes,
            );
            let col2_id = build_balanced_stack(
                col2_leaves,
                CodePaneOrientation::Vertical,
                split_counter,
                generated_nodes,
    ...
    let root_id = match preset {
        ...
        CodePanePreset::TwoRows => {
            build_fixed_row_grid(&leaf_nodes, 2, &mut split_counter, &mut generated_nodes)
        }
        CodePanePreset::ThreeRows => {
            build_fixed_row_grid(&leaf_nodes, 3, &mut split_counter, &mut generated_nodes)
        }
        CodePanePreset::FourRows => {
            build_fixed_row_grid(&leaf_nodes, 4, &mut split_counter, &mut generated_nodes)
        }
        CodePanePreset::Focus | CodePanePreset::MainLeft => {
            build_focus_layout(&leaf_nodes, &mut split_counter, &mut generated_nodes)
        }
        ...
        CodePanePreset::Equal => {
            build_equal_grid(&leaf_nodes, &mut split_counter, &mut generated_nodes)
        }
        CodePanePreset::Tidy => match leaf_nodes.len() {
            ...
            3 => build_focus_layout(&leaf_nodes, &mut split_counter, &mut generated_nodes),
            4..=8 => build_fixed_row_grid(&leaf_nodes, 2, &mut split_counter, &mut generated_nodes),
            9..=12 => {
                build_fixed_row_grid(&leaf_nodes, 3, &mut split_counter, &mut generated_nodes)
            }
            13..=16 => {
            ...
            _ => build_focus_layout(&leaf_nodes, &mut split_counter, &mut generated_nodes),
        },
    };
...
mod tests {
    ...
    #[test]
    fn equal_grid_builds_balanced_layouts_for_various_counts() {
        let mut layout = default_layout("ws_1");
        for count in 1..=16 {
            let applied = apply_layout_preset(&layout, CodePanePreset::Equal).unwrap();
            validate_layout(&applied).unwrap();
            let leaves = visual_leaf_order(&applied);
            assert_eq!(leaves.len(), count);
