...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let secret = hex_decode(&env::var("HIVEORY_DISPATCH_SECRET")?)?;
    let dispatch_id = env::var("HIVEORY_DISPATCH_ID")?;
    let lease_generation = env::var("HIVEORY_DISPATCH_LEASE")?.parse::<u64>()?;
    ...
    let sequence = env::args()
        ...
        .transpose()?
        .or_else(|| {
            env::var("HIVEORY_DISPATCH_SEQUENCE")
                .ok()
    ...
    writeln!(stdout, "HIVEORY_EVENT {output}")?;
    stdout.flush()?;
    Ok(())
}
