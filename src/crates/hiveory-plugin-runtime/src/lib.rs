...
use hiveory_persistence::plugin::{HiveoryPluginStore, HiveoryPluginStoreError};
...
#[derive(Debug, Error)]
pub enum HiveoryPluginRuntimeError {
    #[error("plugin store failure: {0}")]
    Store(#[from] HiveoryPluginStoreError),
    #[error("routine store failure: {0}")]
    RoutineStore(#[from] HiveoryRoutineStoreError),
...
#[derive(Clone)]
pub struct HiveoryPluginRuntime {
    store: HiveoryPluginStore,
    routines: HiveoryRoutineStore,
    secrets: HiveorySecretStoreHandle,
    audit: HiveoryAuditLog,
    client: Client,
}

impl HiveoryPluginRuntime {
    pub fn new(
        persistence: hiveory_persistence::HiveoryPersistence,
        secrets: HiveorySecretStoreHandle,
        audit: HiveoryAuditLog,
    ) -> Result<Self, HiveoryPluginRuntimeError> {
        let client = Client::builder()
            ...
            .build()
            .map_err(|error| HiveoryPluginRuntimeError::Request(error.to_string()))?;
        Ok(Self {
            store: HiveoryPluginStore::new(persistence.clone()),
            routines: HiveoryRoutineStore::new(persistence),
            secrets,
            audit,
            client,
        })
    ...
    ) -> Result<Vec<PluginConnectionSummary>, HiveoryPluginRuntimeError> {
    ...
    ) -> Result<PluginConnectionSummary, HiveoryPluginRuntimeError> {
        let secret_ref = if let Some(secret) = request
            .secret_value
            .as_deref()
    ...
    pub async fn update_connection(
        ...
        let new_secret_ref = if let Some(secret) = request
            .secret_value
            .as_deref()
            .filter(|value| !value.is_empty())
        {
    ...
    ) -> Result<AgentPluginGrant, HiveoryPluginRuntimeError> {
    ...
    pub async fn agent_grants(
        &self,
        agent_id: &str,
    ) -> Result<Vec<AgentPluginGrant>, HiveoryPluginRuntimeError> {
    ...
    pub async fn invocations_for_run(
        &self,
        run_id: &str,
    ) -> Result<Vec<PluginInvocationSummary>, HiveoryPluginRuntimeError> {
    ...
    pub async fn test_connection(
        ...
        if !response.status().is_success() {
            return Err(HiveoryPluginRuntimeError::Request(format!(
    ...
    pub async fn dry_run(
        ...
        let connection = self.store.connection(&request.connection_id).await?.ok_or(
            HiveoryPluginRuntimeError::InvalidInput("connection was not found".to_owned()),
        )?;
                ...
                self.audit
                    .record(
                        ...
                        Some(&format!(
                            ...
                            hiveory_approval_fingerprint(&qualified, &target, arguments_json)
                        )),
                    )
                    .await
                    .map_err(|error| HiveoryPluginRuntimeError::Request(error.to_string()))?;
    ...
    async fn send_json_request(
    ...
    ) -> Result<Value, HiveoryPluginRuntimeError> {
        let response = self
        ...
        let body: Value = serde_json::from_slice(&bytes)
            .map_err(|_| HiveoryPluginRuntimeError::InvalidResponse)?;
    ...
    fn request_builder(
        ...
        secret_ref: Option<&str>,
        builder: reqwest::RequestBuilder,
    ) -> Result<reqwest::RequestBuilder, HiveoryPluginRuntimeError> {
        if matches!(connection.kind, PluginConnectionKind::ApiKeyHeader) {
            let header = connection.api_key_header.as_deref().ok_or(
                HiveoryPluginRuntimeError::InvalidInput("API key header is missing".to_owned()),
            )?;
            let name = HeaderName::from_bytes(header.as_bytes()).map_err(|_| {
                HiveoryPluginRuntimeError::InvalidInput("API key header is invalid".to_owned())
            })?;
            let secret = secret_ref.ok_or(HiveoryPluginRuntimeError::Secret(
    ...
    fn safe_url(
        ...
        let base = Url::parse(origin).map_err(|_| {
            HiveoryPluginRuntimeError::InvalidInput("connection origin is invalid".to_owned())
        })?;
        ...
        let target = base.join(path).map_err(|_| {
            HiveoryPluginRuntimeError::InvalidInput("plugin path could not be resolved".to_owned())
        })?;
    ...
    async fn enabled_manifest(
        &self,
        plugin_id: &str,
    ) -> Result<PluginManifest, HiveoryPluginRuntimeError> {
        let entry = self
            .store
            .catalog()
            .await?
            ...
            .ok_or_else(|| HiveoryPluginRuntimeError::NotFound(plugin_id.to_owned()))?;
...
#[async_trait]
impl HiveoryExternalToolProvider for HiveoryPluginRuntime {
...
) -> Result<Vec<u8>, HiveoryPluginRuntimeError> {
    if response.content_length().unwrap_or(0) > MAX_RESPONSE_BYTES as u64 {
        return Err(HiveoryPluginRuntimeError::InvalidResponse);
    }
    let mut stream = response.bytes_stream();
    let mut bytes = Vec::new();
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|error| HiveoryPluginRuntimeError::Request(error.to_string()))?;
...
fn parse_object(arguments_json: &str) -> Result<Map<String, Value>, HiveoryPluginRuntimeError> {
    ...
    let value: Value = serde_json::from_str(arguments_json).map_err(|_| {
        HiveoryPluginRuntimeError::InvalidInput("plugin arguments are not valid JSON".to_owned())
    })?;
    value
        .as_object()
        .cloned()
...
fn validate_tool_arguments(
    ...
    let schema: Value = serde_json::from_str(&tool.input_schema_json).map_err(|_| {
        HiveoryPluginRuntimeError::InvalidInput("plugin input schema is invalid".to_owned())
    })?;
    ...
    for key in args.keys() {
        if !properties.contains_key(key) {
            return Err(HiveoryPluginRuntimeError::InvalidInput(format!(
...
fn path_arg(args: &Map<String, Value>) -> Result<&str, HiveoryPluginRuntimeError> {
    args.get("path")
...
fn reject_private_host(host: &str) -> Result<(), HiveoryPluginRuntimeError> {
...
fn secret_error(error: HiveorySecretStoreError) -> HiveoryPluginRuntimeError {
